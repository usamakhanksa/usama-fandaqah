<template>
  <div class="p-8 space-y-6 bg-[#f8f9fa] min-h-full">
    <!-- Header Section -->
    <div class="flex flex-col md:flex-row justify-between items-end gap-6 bg-white p-8 rounded-3xl shadow-sm border border-slate-50">
      <div>
    ...
    <!-- Filters & View Toggle -->
    <div class="bg-white p-6 rounded-3xl border border-slate-100 shadow-sm flex flex-wrap gap-4 items-center">
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
       <div v-for="lead in leads" :key="lead.id" class="bg-white p-8 rounded-3xl border border-slate-50 shadow-sm hover:shadow-2xl hover:-translate-y-2 transition-all group overflow-hidden relative">
          ...
          <div class="flex gap-3 pt-2">
             <a :href="'mailto:' + lead.email" class="flex-1 py-4 bg-[#2a273c] text-white rounded-2xl text-xs font-black text-center shadow-lg shadow-slate-100 hover:bg-[#e95a54] transition-all">{{ $t('leads.contacted') }}</a>
             <button @click="updateStatus(lead, 'qualified')" class="flex-1 py-4 bg-[#f2f0eb] text-[#2a273c] rounded-2xl text-xs font-black hover:bg-emerald-50 hover:text-emerald-600 transition-all">{{ $t('leads.qualified') }}</button>
          </div>
       </div>
    </div>
  </div>
