<template>
  <div class="p-6 space-y-6 bg-[#f8f9fa] min-h-full flex flex-col">
    <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-50 flex items-center justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-[#2a273c]">Category Services</h1>
        <p class="text-xs font-black text-[#e95a54] uppercase tracking-widest mt-1">Category–Service Mapping</p>
      </div>
        ...
        <table class="w-full border-collapse">
          <thead>
            <tr class="bg-slate-50/50 border-b border-slate-50">
              ...
              <th class="p-4 text-start"><span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Included</span></th>
              <th class="p-4 text-start"><span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Price Override</span></th>
          ...
          <tbody class="divide-y divide-slate-50">
            <tr v-for="m in mappings" :key="m.id" class="hover:bg-slate-50/30 transition-colors group">
              <td class="p-4"><span class="text-sm font-bold text-[#2a273c]">{{ m.unit_category?.name || m.unit_category_id }}</span></td>
              <td class="p-4"><span class="text-sm text-slate-600">{{ m.service?.name || m.service_id }}</span></td>
              <td class="p-4"><span :class="m.is_included ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500'" class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest">{{ m.is_included ? 'Yes' : 'No' }}</span></td>
              <td class="p-4"><span class="text-xs font-bold text-[#e95a54]">{{ m.price_override ? m.price_override + ' SAR' : '—' }}</span></td>
            ...
            <tr v-if="!mappings.length"><td colspan="6" class="p-16 text-center text-slate-400 text-sm">No mappings found.</td></tr>
          </tbody>
        </table>
      </div>
    </div>
    ...
    <div v-if="showForm" class="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div class="bg-white rounded-3xl p-8 w-full max-w-md space-y-4">
        <h3 class="text-xl font-bold text-[#2a273c]">Add Category–Service Mapping</h3>
        <div><label class="label">Category ID *</label><input v-model="form.unit_category_id" type="number" class="input" placeholder="Category ID"></div>
        <div><label class="label">Service ID *</label><input v-model="form.service_id" type="number" class="input" placeholder="Service ID"></div>
        <div><label class="label">Is Included</label><select v-model="form.is_included" class="input"><option :value="true">Yes</option><option :value="false">No</option></select></div>
        <div><label class="label">Price Override (SAR)</label><input v-model="form.price_override" type="number" class="input" placeholder="Leave blank to use default"></div>
...
const form = reactive({ unit_category_id: '', service_id: '', is_included: false, price_override: '', is_active: true });

const resetForm = () => Object.assign(form, { unit_category_id: '', service_id: '', is_included: false, price_override: '', is_active: true });

const load = async () => {
  const { data } = await api.get('/rooms-module/unit-category-services');
...
const save = async () => {
  ...
  try {
    await api.post('/rooms-module/unit-category-services', form);
...
const remove = async (m) => {
  ...
  await api.delete(`/rooms-module/unit-category-services/${m.id}`);
  load();
};
