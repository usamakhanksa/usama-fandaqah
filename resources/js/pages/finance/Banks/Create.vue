  ...
  <div class="p-6 bg-slate-50 min-h-screen">
    <div class="max-w-3xl mx-auto">
      <div class="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 class="text-2xl font-bold text-slate-800">{{ $t('Add New Bank') }}</h1>
          <p class="text-slate-500 text-sm">{{ $t('Register a new bank account in the system') }}</p>
        </div>
      </div>
...
function submit() {
  form.post(route('finance.banks.store'));
}
</script>
...
.focus\:ring-primary\/20:focus { --tw-ring-color: rgba(233, 90, 84, 0.2); }
</style>
