  ...
  <div class="p-6 bg-slate-50 min-h-screen">
    <div class="max-w-3xl mx-auto">
      <div class="flex items-center gap-4 mb-6">
        ...
        <div>
          <h1 class="text-2xl font-bold text-slate-800">{{ $t('Edit Sender') }}</h1>
          <p class="text-slate-500 text-sm">{{ $t('Update sender information') }}</p>
        </div>
      </div>
...
const form = useForm({
  ...
  name_ar: props.sender.name_ar,
  type: props.sender.type,
  id_number: props.sender.id_number,
  phone: props.sender.phone,
  email: props.sender.email,
  address: props.sender.address,
  bank_id: props.sender.bank_id,
  is_active: props.sender.is_active,
  notes: props.sender.notes,
});

function submit() {
  form.put(route('finance.senders.update', props.sender.id));
}
</script>
