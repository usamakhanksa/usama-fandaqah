<template>
  <div class="p-6 bg-[#f8f9fa] min-h-full space-y-6">
    <div class="flex justify-between items-center">
      <div>
        <h1 class="text-3xl font-bold text-[#2a273c]">{{ isEdit ? 'Edit Schedule' : 'Create Schedule' }}</h1>
        <p class="text-slate-500">Schedule automatic report delivery</p>
      </div>
    </div>
...
const saveSchedule = () => {
  const url = isEdit.value ? `/reports/report-schedules/${props.schedule.id}` : '/reports/report-schedules';
