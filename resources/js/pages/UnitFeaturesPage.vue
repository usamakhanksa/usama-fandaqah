<template>
  <div class="p-6 space-y-6 bg-[#f8f9fa] min-h-full flex flex-col">
    <div class="bg-white p-6 rounded-3xl shadow-sm border border-slate-50 flex items-center justify-between gap-4">
      <div>
        <h1 class="text-2xl font-bold text-[#2a273c]">Unit Features</h1>
        <p class="text-xs font-black text-[#e95a54] uppercase tracking-widest mt-1">Room Feature Settings</p>
      </div>
        ...
        <table class="w-full border-collapse">
          <thead>
            <tr class="bg-slate-50/50 border-b border-slate-50">
              ...
              <th class="p-4 text-start"><span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Icon</span></th>
          ...
          <tbody class="divide-y divide-slate-50">
            <tr v-for="f in features" :key="f.id" class="hover:bg-slate-50/30 transition-colors group">
              <td class="p-4"><span class="text-sm font-bold text-[#2a273c]">{{ f.name }}</span></td>
              <td class="p-4"><span class="text-xs text-slate-500">{{ f.icon || '—' }}</span></td>
              <td class="p-4"><span :class="f.active ? 'bg-emerald-100 text-emerald-600' : 'bg-slate-100 text-slate-500'" class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest">{{ f.active ? 'Active' : 'Inactive' }}</span></td>
              <td class="p-4 text-end">
                <div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button @click="edit(f)" class="bg-slate-100 text-[#2a273c] px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest hover:bg-slate-200">Edit</button>
                  <button @click="remove(f)" class="bg-red-100 text-red-600 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest">Delete</button>
                </div>
              </td>
            </tr>
            <tr v-if="!features.length"><td colspan="4" class="p-16 text-center text-slate-400 text-sm">No features found.</td></tr>
          </tbody>
        </table>
      </div>
    </div>
    ...
    <div v-if="showForm" class="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
      <div class="bg-white rounded-3xl p-8 w-full max-w-md space-y-4">
        <h3 class="text-xl font-bold text-[#2a273c]">{{ editing ? 'Edit Feature' : 'Add Feature' }}</h3>
        <div><label class="label">Name *</label><input v-model="form.name" class="input" placeholder="Feature name"></div>
        <div><label class="label">Icon</label><input v-model="form.icon" class="input" placeholder="icon-name or URL"></div>
...
const form = reactive({ name: '', icon: '', description: '', active: true });

const resetForm = () => Object.assign(form, { name: '', icon: '', description: '', active: true });

const load = async () => {
  const { data } = await api.get('/rooms-module/unit-features');
  features.value = data.data || [];
};

const edit = (f) => { editing.value = f; Object.assign(form, f); showForm.value = true; };

const save = async () => {
  processing.value = true;
  try {
    editing.value
      ? await api.put(`/rooms-module/unit-features/${editing.value.id}`, form)
      : await api.post('/rooms-module/unit-features', form);
...
const remove = async (f) => {
  ...
  await api.delete(`/rooms-module/unit-features/${f.id}`);
  load();
};
