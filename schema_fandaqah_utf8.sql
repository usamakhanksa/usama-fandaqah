...
CREATE TABLE IF NOT EXISTS `cashier_shifts` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `team_id` int(10) unsigned NOT NULL,
  `user_id` int(10) unsigned NOT NULL COMMENT 'Cashier ID',
...
CREATE TABLE IF NOT EXISTS `companies` (
  ...
  `payment_terms_days` int(11) NOT NULL DEFAULT '0' COMMENT 'Options: 0,30,45,60,90,180. Default 0 = payment due immediately.',
...
CREATE TABLE IF NOT EXISTS `company_groups` (
  ...
  `payment_terms_days` int(11) NOT NULL DEFAULT '0' COMMENT '0,30,45,60,90,180 days',
  `created_at` timestamp NULL DEFAULT NULL,
  `updated_at` timestamp NULL DEFAULT NULL,
  `deleted_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`)
...
CREATE TABLE IF NOT EXISTS `customer` (
  ...
  `coming_away` int(11) DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `daily_reports` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `team_id` int(10) unsigned DEFAULT NULL,
  `rented_units` int(10) unsigned NOT NULL DEFAULT '0',
  `vacant_units` int(10) unsigned NOT NULL DEFAULT '0',
  `occupancy_ratio` double NOT NULL DEFAULT '0',
  `new_reservations` int(11) NOT NULL DEFAULT '0',
  `still_reservations` int(11) NOT NULL DEFAULT '0',
  `checked_out_reservations` int(11) NOT NULL DEFAULT '0',
  ...
  `promissory_notes` int(11) NOT NULL DEFAULT '0',
...
CREATE PROCEDURE `get_aggregate_history_forecast`(
            IN startDate DATE, IN endDate DATE, IN teamId INT
        )
BEGIN
            SELECT COUNT(DISTINCT unit_id) AS `occupied_units`, DATE(date_in_time) AS date_time FROM reservations
                WHERE date_in_time >= startDate AND date_in_time <= endDate AND status = "confirmed" AND team_id = teamId
                GROUP BY date_in_time
                ORDER BY date_in_time;
SELECT COUNT(DISTINCT unit_id) AS `arrival_units`, DATE(date_in_time) AS date_time FROM reservations
                WHERE date_in_time >= startDate AND date_in_time <= endDate AND checking_in = 0 AND checked_out IS NULL AND status = "confirmed" AND team_id = teamId
                GROUP BY date_in_time
                ORDER BY date_in_time;
SELECT 0 AS `complimentary_rooms`, DATE(date_in_time) AS date_time FROM reservations
                WHERE date_in_time >= startDate AND date_in_time <= endDate AND checking_in = 0 AND checked_out IS NULL AND status = "confirmed" AND team_id = teamId
                GROUP BY date_in_time
                ORDER BY date_in_time;
SELECT 0 AS `house_use_rooms`, DATE(date_in_time) AS date_time FROM reservations
                WHERE date_in_time >= startDate AND date_in_time <= endDate AND checking_in = 0 AND checked_out IS NULL AND status = "confirmed" AND team_id = teamId
                GROUP BY date_in_time
                ORDER BY date_in_time;
SELECT COUNT(DISTINCT unit_id) AS `deducted_individuals`, DATE(date_in_time) AS date_time FROM reservations
                WHERE date_in_time >= startDate AND date_in_time <= endDate AND status = "confirmed" AND company_id IS NULL AND team_id = teamId
                GROUP BY date_in_time
                ORDER BY date_in_time;
SELECT COUNT(DISTINCT unit_id) AS `deducted_group`, DATE(date_in_time) AS date_time FROM reservations
                WHERE date_in_time >= startDate AND date_in_time <= endDate AND status = "confirmed" AND company_id IS NOT NULL AND team_id = teamId
                GROUP BY date_in_time
                ORDER BY date_in_time;
SELECT (COUNT(DISTINCT unit_id) * 100 / (SELECT COUNT(id) FROM units WHERE team_id = teamId AND status = 1)) AS `occupied_units_percentage`,
                (SELECT COUNT(id) FROM units WHERE team_id = teamId AND status = 1) AS total_units,
                COUNT(DISTINCT unit_id) AS occupied_units, DATE(date_in_time) AS date_time FROM reservations
                WHERE date_in_time >= startDate AND date_in_time <= endDate AND status = "confirmed" AND team_id = teamId
                GROUP BY date_in_time
                ORDER BY date_in_time;
SELECT COUNT(DISTINCT unit_id) AS `occupied_units`, FORMAT(SUM(sub_total), 2) AS `revenue`, DATE(date_in_time) AS date_time FROM reservations
                WHERE date_in_time >= startDate AND date_in_time <= endDate AND status = "confirmed" AND team_id = teamId
                GROUP BY date_in_time
                ORDER BY date_in_time;
SELECT COUNT(DISTINCT unit_id) AS `occupied_units`, FORMAT(SUM(sub_total), 2) AS `revenue`, 
                (SUM(sub_total) / (SELECT COUNT(id) FROM units WHERE team_id = teamId AND status = 1)) AS `average_revenue_per_unit`, DATE(date_in_time) AS date_time FROM reservations
                WHERE date_in_time >= startDate AND date_in_time <= endDate AND status = "confirmed" AND team_id = teamId
                GROUP BY date_in_time
                ORDER BY date_in_time;
SELECT COUNT(DISTINCT unit_id) AS `departure_rooms`, DATE(date_in_time) AS date_time FROM reservations
                WHERE date_out_time >= startDate AND date_out_time <= endDate AND status = "confirmed" AND team_id = teamId
                GROUP BY date_in_time
                ORDER BY date_in_time;
SELECT 0 AS `day_use_rooms`, DATE(date_in_time) AS date_time FROM reservations
                WHERE date_out_time >= startDate AND date_out_time <= endDate AND status = "confirmed" AND team_id = teamId
                GROUP BY date_in_time
                ORDER BY date_in_time;
SELECT 0 AS `no_show_rooms`, DATE(date_in_time) AS date_time FROM reservations
                WHERE date_out_time >= startDate AND date_out_time <= endDate AND status = "confirmed" AND team_id = teamId
                GROUP BY date_in_time
                ORDER BY date_in_time;
SELECT COUNT(DISTINCT id) AS `out_of_order_rooms`, DATE(created_at) AS date_time FROM units 
                WHERE created_at >= startDate AND created_at <= endDate AND status = 0 AND team_id = teamId
                GROUP BY created_at
                ORDER BY created_at;
SELECT (COUNT(*) + COUNT(guests.id)) AS `res_guests`, DATE(date_in_time) AS date_time FROM reservations
                LEFT JOIN guests
                ON reservations.id = guests.reservation_id
                WHERE reservations.date_in_time >= startDate AND reservations.date_in_time <= endDate AND reservations.status = "confirmed" AND reservations.team_id = teamId
                GROUP BY reservations.date_in_time
                ORDER BY reservations.date_in_time;
        END;


CREATE PROCEDURE `get_aggregate_housekeeping_discrepancy`(
            IN teamId INT
        )
BEGIN
            SELECT un.unit_category_id, uc.name AS category_name, un.name AS unit_name, un.unit_number, un.`status`,
                (SELECT COUNT(*) FROM reservations WHERE unit_id = un.id AND checking_in = 1 AND checked_out IS NULL) AS `occupied_total`
                FROM units un
                LEFT JOIN unit_categories uc
                ON
                un.unit_category_id = uc.id
                WHERE un.team_id = teamId;
        END;


CREATE PROCEDURE `get_aggregate_no_show_cancellation`(
	IN `startDate` DATE,
	IN `endDate` DATE,
	IN `teamId` INT,
	IN `pageNo` INT,
...
BEGIN
		    	...
		    	reservations rsv
		    	WHERE
               rsv.team_id			= teamId      AND
               rsv.date_out_time >= startDate   AND
               rsv.date_out_time <= endDate    AND
               rsv.`status`      = "confirmed" AND
               rsv.checked_in   IS  NULL) + (SELECT 
		    		COUNT(*)
					FROM 
		    		reservations rsv
		    		WHERE
               ...
               rsv.date_out_time >= startDate   AND
               rsv.date_out_time <= endDate    AND
               rsv.`status`      = "canceled") AS total_records;
            ...
            SELECT (SELECT 
               ...
               rsv.date_out_time >= startDate   AND
               rsv.date_out_time <= endDate    AND
               rsv.`status`      = "canceled") AS total_pages;
		    	
            (SELECT rsv.number, cust.name, "NS" AS C_N, un.name AS unit_name,
                rsv.sub_total AS base_rate, rsv.date_in AS arrive, rsv.date_out AS depart,
                (SELECT COUNT(*) FROM guests gs WHERE gs.reservation_id = rsv.id ) + 1 AS total_guests,
                rsv.checked_in
                FROM reservations rsv
                LEFT JOIN customer cust
                ON
                rsv.customer_id = cust.id
                INNER JOIN units un
                ON
                rsv.unit_id = un.id
                WHERE
                ...
                rsv.date_out_time <= endDate    AND
                rsv.`status`      = "confirmed" AND
                rsv.checked_in   IS  NULL		  AND
                rsv.customer_id  IS NOT NULL)
                
                UNION ALL
                (SELECT rsv.number, comp.name, "NS" AS C_N, un.name AS unit_name,
                rsv.sub_total AS base_rate, rsv.date_in AS arrive, rsv.date_out AS depart,
                (SELECT COUNT(*) FROM guests gs WHERE gs.reservation_id = rsv.id ) + 1 AS total_guests,
                rsv.checked_in
                FROM reservations rsv
                ...
                rsv.`status`      = "confirmed" AND
                rsv.checked_in   IS  NULL		  AND
                rsv.company_id  IS NOT NULL)
                
                UNION ALL
                (SELECT rsv.number, cust.name, "C" AS C_N, un.name AS unit_name,
                rsv.sub_total AS base_rate, rsv.date_in AS arrive, rsv.date_out AS depart,
                (SELECT COUNT(*) FROM guests gs WHERE gs.reservation_id = rsv.id ) + 1 AS total_guests,
                rsv.checked_in
                FROM reservations rsv
                ...
                WHERE
                rsv.team_id			= teamId      AND
                rsv.date_in_time >= startDate   AND
                rsv.date_in_time <= endDate    AND
                rsv.`status`      = "canceled" AND
                rsv.customer_id  IS NOT NULL)
                
                UNION ALL
                (SELECT rsv.number, comp.name, "C" AS C_N, un.name AS unit_name,
                rsv.sub_total AS base_rate, rsv.date_in AS arrive, rsv.date_out AS depart,
                (SELECT COUNT(*) FROM guests gs WHERE gs.reservation_id = rsv.id ) + 1 AS total_guests,
                rsv.checked_in
                FROM reservations rsv
...
CREATE PROCEDURE `get_aggregate_paid_outs`(
            IN startDate DATE, IN endDate DATE, IN teamId INT
        )
BEGIN
            SELECT 
        SUM(rsv.sub_total) AS `sub_total`,
        SUM(rsv.ewa_total) AS `ewa_price`,
        SUM(rsv.vat_total) AS `vat_total`
        FROM reservations rsv
        WHERE rsv.team_id = teamId 			            AND 
                rsv.date_in_time >= startDate 	    	AND
                rsv.date_in_time < endDate 	 	 		AND
                rsv.`status`    = "confirmed";
        
        SELECT 
        SUM(JSON_EXTRACT(srv.meta, "$.sub_total")) AS `sub_total`,
        SUM(JSON_EXTRACT(srv.meta, "$.vat_total")) AS `vat_total`
        FROM service_logs srv
        WHERE 
                srv.team_id     = teamId		    AND
                srv.created_at >= startDate 	    AND
                srv.created_at <  endDate;
        
        #Payment Reservations
        SELECT 
        trx.meta_payment_type,
            SUM(IF (trx.`type` = "deposit", ABS(trx.amount / 100), 0)) AS `payment_reservations`
        FROM reservations rsv
        LEFT JOIN transactions trx
        ON
        rsv.id = trx.payable_id
        WHERE       rsv.team_id = teamId 			AND 
                    rsv.date_in_time >= startDate 	AND
                    rsv.date_in_time < endDate      AND
                    trx.`type`       = "deposit"
            GROUP BY trx.meta_payment_type;
    
        #Payment Services
        SELECT 
            trx.meta_payment_type,
            SUM(IF (trx.`type` = "deposit", ABS(trx.amount / 100), 0)) AS `payment_services`
            FROM service_logs srv
            LEFT JOIN transactions trx
        ON
        srv.transaction_id = trx.id
        WHERE srv.team_id = teamId 				        AND 
                    srv.created_at >= startDate 		AND
                    srv.created_at <  endDate      	    AND
                    trx.`type`        = "deposit"
            GROUP BY trx.meta_payment_type;
        END;


CREATE PROCEDURE `get_aggregate_trial_balance`(
            IN startDate DATE, IN endDate DATE, IN teamId INT
        )
BEGIN
            SELECT 
...
CREATE TABLE IF NOT EXISTS `highlightables` (
  ...
  `highlightable_id` int(11) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `invoice_credit_notes` (
  ...
  `service_log_id` int(10) unsigned DEFAULT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `invoice_transfers` (
  ...
  `reservation_invoice_id` int(10) unsigned DEFAULT NULL COMMENT 'The formal invoice document being transferred',
  `promissory_id` int(10) unsigned NOT NULL COMMENT 'FK to promissories.id â€” the AR record created',
...
CREATE TABLE IF NOT EXISTS `night_audit_log` (
  ...
  `noshows_flagged` int(11) NOT NULL DEFAULT '0',
  `noshow_charges_posted` int(11) NOT NULL DEFAULT '0',
  `transactions_frozen` int(11) NOT NULL DEFAULT '0',
...
CREATE TABLE IF NOT EXISTS `night_audit_noshow_log` (
  ...
  `charge_transaction_id` int(10) unsigned DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `night_audit_occupancy_snapshot` (
  ...
  `rooms_occupied` int(11) NOT NULL,
  `rooms_cleaning` int(11) NOT NULL,
  `rooms_maintenance` int(11) NOT NULL,
  `rooms_complimentary` int(11) NOT NULL,
  `rooms_house_use` int(11) NOT NULL,
  `rooms_day_use` int(11) NOT NULL,
  ...
  `arrivals_count` int(11) NOT NULL,
  `departures_count` int(11) NOT NULL,
  `stayovers_count` int(11) NOT NULL,
  `noshows_count` int(11) NOT NULL,
  `cancellations_count` int(11) NOT NULL,
  `new_bookings_count` int(11) NOT NULL,
...
CREATE TABLE IF NOT EXISTS `no_show_charge_rules` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `team_id` int(10) unsigned NOT NULL COMMENT 'FK to teams (Hotel)',
...
CREATE TABLE IF NOT EXISTS `online_reservations` (
  ...
  `public_api_consumer_id` int(10) unsigned DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `plans` (
  ...
  `default_keys` int(11) NOT NULL DEFAULT '0',
  `additional_key_price` double NOT NULL DEFAULT '0',
  `allowed_number_of_users` int(11) NOT NULL DEFAULT '0',
...
CREATE TABLE IF NOT EXISTS `promissories` (
  ...
  `updated_at` timestamp NULL DEFAULT NULL,
  `deleted_at` timestamp NULL DEFAULT NULL,
  `company_id` int(10) unsigned DEFAULT NULL COMMENT 'Direct FK to companies.id. Avoids join through reservation.',
...
CREATE TABLE IF NOT EXISTS `reservations` (
  ...
  `updated_at` timestamp NULL DEFAULT NULL,
  `deleted_at` timestamp NULL DEFAULT NULL,
  `purpose_of_visit` int(11) DEFAULT NULL,
  ...
  `rent_type` int(11) NOT NULL DEFAULT '1',
...
CREATE TABLE IF NOT EXISTS `reservation_service_mappers` (
  ...
  `reservation_service_id` int(10) unsigned DEFAULT NULL,
  `created_at` timestamp NULL DEFAULT NULL,
  `updated_at` timestamp NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `roles` (
  ...
  `team_id` int(10) unsigned DEFAULT NULL,
  `name` varchar(255) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `deletable` int(11) NOT NULL DEFAULT '0',
  `created_at` timestamp NULL DEFAULT NULL,
  `updated_at` timestamp NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `subscription_items` (
  `id` bigint(20) unsigned NOT NULL AUTO_INCREMENT,
  `team_subscription_id` int(10) unsigned NOT NULL,
  `stripe_id` varchar(191) COLLATE utf8mb4_unicode_ci NOT NULL,
  `stripe_plan` varchar(191) COLLATE utf8mb4_unicode_ci NOT NULL,
...
CREATE TABLE IF NOT EXISTS `teams` (
  ...
  `deleted_at` timestamp NULL DEFAULT NULL,
  `country_code` int(10) unsigned NOT NULL DEFAULT '113',
  ...
  `sure_bills_client_id` int(11) DEFAULT NULL,
  ...
  `allowed_units_count` int(11) DEFAULT NULL,
  ...
  `last_night_audit_by` int(10) unsigned DEFAULT NULL COMMENT 'User ID or NULL if auto.',
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `team_counters` (
  ...
  `last_reservation_number` int(11) NOT NULL DEFAULT '0',
  ...
  `last_receipt_number` int(11) NOT NULL DEFAULT '0',
  `payment_number` int(11) NOT NULL DEFAULT '0',
  `last_payment_number` int(11) NOT NULL DEFAULT '0',
  `contract_number` int(11) NOT NULL DEFAULT '0',
  `last_contract_number` int(11) NOT NULL DEFAULT '0',
  `promissory_number` int(10) unsigned DEFAULT '0',
  `last_promissory_number` int(10) unsigned DEFAULT '0',
  `created_at` timestamp NULL DEFAULT NULL,
  `updated_at` timestamp NULL DEFAULT NULL,
  `service_number` int(11) NOT NULL DEFAULT '0',
  `last_service_number` int(11) NOT NULL DEFAULT '0',
  `credit_note_number` int(11) NOT NULL DEFAULT '0',
  `last_credit_note_number` int(11) NOT NULL DEFAULT '0',
  PRIMARY KEY (`id`)
...
CREATE TABLE IF NOT EXISTS `team_subscriptions` (
  ...
  `unit_counts` int(11) NOT NULL DEFAULT '100',
...
CREATE TABLE IF NOT EXISTS `transactions` (
  ...
  `meta` longtext CHARACTER SET utf8mb4 COLLATE utf8mb4_bin,
  `receiver_bank_id` int(11) DEFAULT NULL,
  ...
  `team_id` bigint(20) DEFAULT NULL,
  `correction_of_transaction_id` int(10) unsigned DEFAULT NULL COMMENT 'FK to transactions.id â€” links reversal to original.',
...
CREATE TABLE IF NOT EXISTS `turnaway_logs` (
  ...
  `created_by` int(10) unsigned NOT NULL,
  `customer_id` bigint(20) unsigned DEFAULT NULL,
  `turnaway_reason_id` int(10) unsigned NOT NULL,
  `description` varchar(191) COLLATE utf8mb4_unicode_ci DEFAULT NULL,
  `created_at` timestamp NULL DEFAULT NULL,
  `updated_at` timestamp NULL DEFAULT NULL,
  `deleted_at` timestamp NULL DEFAULT NULL,
  ...
  CONSTRAINT `turnaway_logs_customer_id_foreign` FOREIGN KEY (`customer_id`) REFERENCES `customer` (`id`),
...
CREATE TABLE IF NOT EXISTS `units` (
  ...
  `gates_lock_id` int(11) DEFAULT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `unit_categories` (
  ...
  `number_of_beds` int(11) DEFAULT '0',
...
CREATE TABLE IF NOT EXISTS `website_settings` (
  ...
  `deposit_percentage` int(11) NOT NULL DEFAULT '100',
  ...
  `time_payment_completed` int(11) DEFAULT '10',
  ...
  `time_payment_completed_hours` int(11) NOT NULL DEFAULT '0',
