...
class Team extends SparkTeam implements Wallet
{
    ...
    public static function boot()
    {
        ...
        static::created(function ($query) {
            $role = \App\Models\Role::where('slug', 'admin')->where('team_id', $query->id)->first();
            if (!$role) {
            ...
            $staff = \App\Models\Role::where('slug', 'staff')->where('team_id', $query->id)->first();
            if (!$staff) {
            ...
            $housekeeping = \App\Models\Role::where('slug', 'housekeeping')->where('team_id', $query->id)->first();
            if (!$housekeeping) {
                $housekeeping = new \App\Models\Role();
    ...
    {
        return $this->hasOne(\App\TeamCounter::class);
    }
