    ...
    protected static function bootLogsActivity()
    {
        static::eventsToBeRecorded()->each(function ($eventName) {
            return static::$eventName(function (Model $model) use ($eventName) {
                ...
                $currentTeam = auth()->user() ? auth()->user()->currentTeam : null;
                // Don't log anything unless the tenant has enable_activity_logs enabled from control
                if($currentTeam && !$currentTeam->enable_activity_logs){
                   return;
                }
