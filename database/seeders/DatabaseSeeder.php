...
class DatabaseSeeder extends Seeder {
  public function run(): void {
    ...
    for($u=2;$u<=22;$u++){DB::table('users')->insert(['role_id'=>rand(1,5),'name'=>fake()->name(),'email'=>"user{$u}@hotel.test",'password'=>Hash::make('password'),'avatar'=>'/assets/avatars/guest'.(($u%8)+1).'.svg','last_seen_at'=>now()->subHours(rand(0,30)),'created_at'=>now(),'updated_at'=>now()]);}

    for($f=1;$f<=6;$f++){DB::table('room_floors')->insert(['name'=>"Floor {$f}",'level'=>$f,'created_at'=>now(),'updated_at'=>now()]);}
    for($i=1;$i<=6;$i++){DB::table('room_types')->insert(['name'=>["Twin Room","Deluxe","Suite","Family","Studio","Executive"][$i-1],'base_price'=>rand(80,350),'created_at'=>now(),'updated_at'=>now()]);}
    ...
    for($i=1;$i<=34;$i++){DB::table('units')->insert(['name'=>"Unit $i",'number'=>'U'.str_pad((string)$i,3,'0',STR_PAD_LEFT),'status'=>fake()->randomElement(['active','inactive']),'created_at'=>now(),'updated_at'=>now()]);}
    for($i=1;$i<=130;$i++){DB::table('guests')->insert(['name'=>fake()->name(),'email'=>fake()->safeEmail(),'phone'=>fake()->phoneNumber(),'avatar'=>'/assets/avatars/guest'.(($i%8)+1).'.svg','created_at'=>now(),'updated_at'=>now()]);}
    ...
    for($i=1;$i<=260;$i++){ $checkIn=now()->subDays(rand(1,15))->addDays(rand(0,30)); $checkOut=(clone $checkIn)->addDays(rand(1,5)); DB::table('reservations')->insert(['code'=>'RSV'.str_pad((string)$i,5,'0',STR_PAD_LEFT),'guest_id'=>rand(1,130),'room_id'=>rand(1,68),'unit_id'=>rand(1,34),'reservation_status_id'=>rand(1,5),'check_in'=>$checkIn,'check_out'=>$checkOut,'stay_type'=>rand(0,1)?'checkin':'checkout','created_at'=>now(),'updated_at'=>now()]); }
    for($i=1;$i<=165;$i++){ DB::table('bookings')->insert(['reservation_id'=>$i,'guest_id'=>rand(1,130),'room_id'=>rand(1,68),'check_in'=>now()->addDays(rand(0,20)),'check_out'=>now()->addDays(rand(21,30)),'total_amount'=>rand(200,1800),'created_at'=>now(),'updated_at'=>now()]); }
    for($i=1;$i<=165;$i++){DB::table('payments')->insert(['booking_id'=>$i,'amount'=>rand(100,1800),'method'=>['cash','card','transfer'][array_rand([0,1,2])],'created_at'=>now(),'updated_at'=>now()]); DB::table('invoices')->insert(['booking_id'=>$i,'number'=>'INV'.str_pad((string)$i,5,'0',STR_PAD_LEFT),'amount'=>rand(100,1800),'status'=>fake()->randomElement(['paid','pending','unpaid','failed','refund']),'created_at'=>now(),'updated_at'=>now()]);}
    ...
    for($i=1;$i<=45;$i++){DB::table('service_orders')->insert(['service_id'=>rand(1,15),'guest_id'=>rand(1,130),'qty'=>rand(1,5),'created_at'=>now(),'updated_at'=>now()]); DB::table('product_orders')->insert(['product_id'=>rand(1,30),'guest_id'=>rand(1,130),'qty'=>rand(1,5),'created_at'=>now(),'updated_at'=>now()]);}
    for($i=1;$i<=60;$i++){DB::table('p_o_s_orders')->insert(['guest_id'=>rand(1,130),'amount'=>rand(20,400),'status'=>fake()->randomElement(['completed','pending','cancelled']),'created_at'=>now(),'updated_at'=>now()]);}
    ...
    for($i=0;$i<95;$i++){ $d=now()->subDays(94-$i)->toDateString();
