...
namespace Surelab\Settings\ValueObjects;
...
use Surelab\Settings\Traits\CacheableTrait;
use Surelab\Settings\Traits\CallableTrait;
use Surelab\Settings\Traits\JsonableTrait;
use ReflectionException;

/**
 * Class SettingRegister
 * @package Surelab\Settings\ValueObjects
 */
final class SettingRegister implements Serializable, JsonSerializable
{
            ...
            foreach ($group->getItems() as $item) {
                foreach($keyValueArray as $key => $value) {
                    if ($key === $item->getKey()) {
                        ...
                        is_array($value) ? $value = \GuzzleHttp\json_encode($value) : $value = $value ;
     ...
     * @return mixed|null|SettingItem
     */
    public static function getGroupByName(string $key)
    {
        $settingsRegister = self::getInstance();
        $groups = $settingsRegister->getGroups();
        foreach ($groups as $group) {
            if ($group->getKey($key) == $key)
                return $group;
        }
