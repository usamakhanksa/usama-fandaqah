<template>
  <div id="photoalbum_settings_page">
    ...
    <div class="block_area">
      <div class="title_area">{{__('Photo album')}}</div>
      <div class="content_area">
        <div class="enable_gallery">
          <label for="enable_gallery">{{__('Activate Album')}}</label>
          <label class="switch">
            <input type="checkbox" id="enable_gallery" ref="checkbox" v-model="enable_photo_album" :checked="checked" @change="enableSliderChanged">
    ...
    <div class="block_area">
      <div class="title_area">{{__('Photos')}}</div>
      <div class="content_area">
    ...
    export default {
        ...
        data() {
            return {
                ...
                thread: 3,
                name: 'file',
                postAction: '/nova-vendor/settings/photo-album-upload-handler',
                headers: {
        ...
        methods: {
                ...
                if (newFile && oldFile) {
                    ...
                    if (newFile.success && !oldFile.success) {
                        ...
                        if(this.handler === this.files.length){
                            ...
                            this.files = [];
                            this.$toasted.success(Nova.app.__('Photos has been attached successfully'), {
                                duration: 3000
                            })
            ...
            getPhotos(from = null){
                ...
                axios.get(`/nova-vendor/settings/photos/${Spark.state.currentTeam.id}`)
                    .then(response => {

                       self.countPhotos = response.data.photos.length;
                       $.each(response.data.photos , function(index , val){
                           self.imagesUrls.push(val.path);
                       })

                       self.images = response.data.photos;
                       self.isImagesLoading = false;

                       // to avoid undefined ref
                       if(from != 'event'){
                           self.$refs.checkbox.checked = response.data.enable_photo_album;
                       }
            ...
            enableSliderChanged(){
                ...
                if(!this.countPhotos){
                    ...
                    this.$toasted.error(Nova.app.__('Sorry , but we can\'t enable album without photos'), {
                        duration: 3000
                    });
                    this.enable_photo_album = false;
                    return;
                }else{

                    axios.post('/nova-vendor/settings/enable-or-disable-photo-album' , {status : this.enable_photo_album})
                            .then((response) => {
                                if(response.data.status){
                                    this.$toasted.success(Nova.app.__('Photo Album has been enabled successfully'), {
                                        duration: 3000
                                    });
                                }else{
                                    this.$toasted.success(Nova.app.__('Photo Album has been disabled successfully'), {
                                        duration: 3000
                                    });
                                }
                            })
