<template>
  <div>
    ...
    <div id="payment_options_settings">
      <div class="title">{{__('Language settings')}}</div>
      <div class="content_page">
        <form @submit.prevent="updateLanguage">
          <div class="choose_options">
            <div class="name">{{__('Active Languages')}}</div>
            <div class="options_area">
              <div class="item">
                <p>{{__('Arabic Language')}}</p>
                <label class="switch">
                  <input type="checkbox"  ref="checkbox1"  v-model="arabic" @change="handleCheckedStatusForArabicLanguage($event)">
                  <span class="slider round"></span>
                </label>
              </div><!-- item -->
              <div class="item">
                <p>{{__('English Language')}}</p>
                <label class="switch">
                  <input type="checkbox"  ref="checkbox2"  v-model="english" @change="handleCheckedStatusForEnglishLanguage($event)">
                  <span class="slider round"></span>
                </label>
              </div><!-- item -->
            </div><!-- options_area -->
          ...
          <div class="block_area">
                <div class="name">{{__('Default Language')}}</div>
                <div class="textarea_area">
                    <select v-model="default_lang">
                        <option value="ar">{{__('Arabic Language')}}</option>
                        <option value="en">{{__('English Language')}}</option>
                    </select>
                </div><!-- textarea_area -->
            </div><!-- block_area -->
...
<script>
    ...
    export default {
        ...
        methods: {
            getSettings() {
                Nova.request()
                    ...
                    .then(response => {
                        ...
                        this.arabic = this.settings.arabic_lang;
                        this.english = this.settings.english_lang;
                        this.default_lang = this.settings.default_lang;
                    })
                    .catch(error => {
                        console.log(error)
                    });
            },
            handleCheckedStatusForArabicLanguage(event){
            ...
            handleCheckedStatusForEnglishLanguage(event){
            ...
            updateLanguage() {
                ...
                if(!this.arabic && !this.english){
                    this.$toasted.error(Nova.app.__('Please you have to enable at least one language option'), {
                        duration: 3000
                    });
                    return false;
                }

                if((this.arabic && !this.english) && this.default_lang === 'en'){
                    this.$toasted.error(Nova.app.__('We can\'t set english language as your default language make sure english option is active'), {
                        duration: 3000
                    });
                    return false;
                }

                if((!this.arabic && this.english) && this.default_lang === 'ar'){
                    this.$toasted.error(Nova.app.__('We can\'t set arabic language as your default language make sure english option is active'), {
                        duration: 3000
                    });
                    return false;
                }
                ...
                Nova.request()
                    .put("/nova-vendor/settings/update-website-settings/"+this.team.id, {
                        arabic_lang : this.arabic ,
                        english_lang : this.english,
