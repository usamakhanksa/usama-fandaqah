<template>
    <div>
        ...
        <div class="title-card title-card p-3 bg-gray-200 text-xl rounded-lg rounded-b-none border-b-2  border-gray-300">{{__('Introductory pages')}}</div>

        <div id="pages">
            <div class="title">
                <span>{{__('Pages')}}<p>( {{paginator && paginator.totalResults ? paginator.totalResults : 0 }} )</p></span>
                <a onclick="Nova.app.$router.push('/settings/website/pages/new')">{{__('Add Page')}}</a>
            </div><!-- title -->
                ...
                <div class="table_responsive">
                    <table>
                        ...
                        <tr>
                            <th>{{__('Page Title')}}</th>
                            <th>{{__('Status')}}</th>
                            <th>{{__('Order of appearance')}}</th>
                            <th>{{__('Actions')}}</th>
                        </tr>
                        </thead>
                        <tbody>
                        <template v-if="collection.length">
                            <tr v-for="(page , i) in collection" :key="i">
                                <td>{{page.title[locale]}}</td>
                                <td>
                                    <span v-if="page.status" class="enabled">{{__('Active')}}</span>
                                    <span v-else class="not_enabled">{{__('Inactive')}}</span>
                                </td>
                                <td>{{page.order}}</td>
                                <td>
                                    <div class="action">
                                        <router-link :to="{name: 'settings.website.pages.details', params: {id: page.id}}" :title="__('View')">
                                        ...
                                        <button type="button" @click="deletePage(page.id)" :title="__('Delete')">
                        ...
                        <template v-else>
                            <tr>
                                <td colspan="7">{{__('No Introductory Pages Found')}}</td>
                            </tr>
                        </template>
                        </tbody>
                    </table>
            ...
            <delete-confirm ref="deleteShared" :id="targetToDelete" from="index" />
        </div>
    </div>
</template>
...
<script>
    ...
    export default {
        ...
        methods : {
            getPages(page = 1){
                this.isLoading = true;
                Nova.request().get(`/nova-vendor/settings/get-pages?page=${page}`)
                    .then(response => {
