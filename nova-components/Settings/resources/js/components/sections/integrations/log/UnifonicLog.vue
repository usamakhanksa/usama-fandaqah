<template>
    <div>
    <sweet-modal ref="shmsLogModal" :enable-mobile-fullscreen="false"  blocking overlay-theme="dark" :title="__('Unifonic Log')" @close="closeLog()">
        <div>
            <div class="shmslog_table" v-if="!logLoading && data.meta.total > 0">
                <table class="table-auto">
                    <thead>
                    <tr>
                        <th class="px-4 py-2">{{__('Status')}}</th>
                        <th class="px-4 py-2">{{__('Senders Numbers')}}</th>
                        <th class="px-4 py-2">{{__('Sms Message')}}</th>
                        <th class="px-4 py-2">{{__('Unifonic Message')}}</th>
                    </tr>
                    </thead>
                    <tbody>
                    <tr v-for="item in data.data" :key="item.id">
                        <td class="border px-4 py-2" :class="'badge-'+jobStatusClass(item.status)">{{
                            __(item.status) }}
                        </td>
                        <td class="border px-4 py-2">{{ item.payload.phone }}</td>
                        <td class="border px-4 py-2" >{{item.payload.message}}</td>
                        <td class="border px-4 py-2">{{ item.response.success ?  __('success') : __('failed') }}</td>
                    </tr>
                    </tbody>
                </table>
            </div>
            <div class="w-full flex flex-wrap mt-8 justify-center" v-if="data.meta.total > data.meta.per_page">
                <pagination
    ...
    export default {
        ...
        methods: {
            ...
            getLog(page = 1) {
                ...
                Nova.request()
                    .get('/nova-vendor/settings/unifonic-log?page=' + page).then(response => {
