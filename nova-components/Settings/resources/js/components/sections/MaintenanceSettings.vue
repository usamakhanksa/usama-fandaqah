<template>
    <div>
        ...
        <div id="reservation_services_card">
            <div class="title">{{ __('Maintenance settings') }}</div>
            <div class="content_page">
                <div class="action_buttons">
                    <button  class="add_receipts" @click="addHandler">{{  __('Add Maintenance Type')  }}</button>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            getData(page = 1){
                this.isLoading = true;
                let url = `/nova-vendor/settings/getMaintenanceSettings?page=${page}`;
                axios.get(url)
                .then(response => {
            ...
            commitAdd(){
                ...
                axios.post(`/nova-vendor/settings/createActionType`, type)
                .then(response => {
            ...
            commitEdit(){
                ...
                axios.post(`/nova-vendor/settings/editActionType`, maintenanceType)
                .then(response => {
                    this.crudLoading = false;
                    this.$toasted.show(this.__('Saved Successfully'), {type: 'success'});
                    this.id = null;
            ...
            commitDelete(){
                this.crudLoading = true;
                axios.post(`/nova-vendor/settings/deleteActionType`, {id : this.id})
                .then(response => {
                   
                    this.crudLoading = false;
                    if(response.data.is_attached){
                        this.$toasted.show(this.__('Can not delete maintenance type because it is attached to the unit'), {type: 'error'});
                        return;
                    }else{
                        this.$toasted.show(this.__('Maintenance type deleted successfully'), {type: 'success'});
                    }
                    this.id = null;
                    this.name_ar = null;
                    this.name_en = null;
