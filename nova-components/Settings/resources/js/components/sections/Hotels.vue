<template>
    <loading-view :loading="initialLoading" :dusk="resourceName + '-index-component'">
        ...
        <p class="mb-3 text-90 font-normal text-2xl">{{__("Facility Information")}}</p>
            ...
            <loading-view :loading="loading">
                <div v-if="!resources.length" class="flex justify-center items-center px-6 py-8">
                    <div class="text-center">
                        ...
                        <p class="text-base text-80 font-normal mb-6">
                            {{
...
<br />
<br />
<!--        <p class="mb-3 text-90 font-normal text-2xl">{{__('General Setting For Current Hotel')}} ( {{spark.state.currentTeam.name}} )</p>-->
        <p class="mb-3 text-90 font-normal text-2xl">{{__('General Settings')}} </p>
        <card class="general_settings">

            <form @submit.prevent="updateSettings">

                <div v-for="(setting,i) in settings.items" :key="i">
                     <template v-if="setting.key === 'automatic_renewal_of_reservations'">
                         <auto-renew-toggle 
                            v-bind:class="`form-${setting.field.textAlign}`"
                            v-bind:field="setting.field"
                            v-bind:v-model="setting.value"
                            :auto_renew_option="setting.value"
                            ref="components"
                         />
                    </template>
                    <template v-else-if="setting.key === 'daily_single_days'">
                         <daily-toggle 
                            v-bind:class="`form-${setting.field.textAlign}`"
                            v-bind:field="setting.field"
                            v-bind:v-model="setting.value"
                     ...
                     <template v-else-if="setting.key === 'monthly_single_days'">
                         <daily-toggle 
                            v-bind:class="`form-${setting.field.textAlign}`"
                            v-bind:field="setting.field"
                            v-bind:v-model="setting.value"
