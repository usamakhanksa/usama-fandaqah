<template>
    ...
    <div id="reservations_management_page" class="relative">
      <div class="title">{{ __("Reservations With Services Included Report") }}</div>
        ...
        <div class="statistics_area relative" v-if="canShowStatistics">
          ...
          <!-- <ul>
            <li>
              ...
              <p>{{ statistics.total_amount }} {{ __(currency) }}</p>
            </li>
            <li>
              <span>{{ __("Total Income") }}</span>
              <p>{{ statistics.total_income }} {{ __(currency) }}</p>
            </li>
            <li>
              <span>{{ __("Total Rent") }}</span>
              <p>{{ statistics.total_rent }} {{ __(currency) }}</p>
            </li>
            <li>
              <span>{{ __("Total Services") }}</span>
              <p>{{ statistics.total_services }} {{ __(currency) }}</p>
            </li>
            <li>
              <span>{{ __("Total Taxes") }}</span>
              <p>{{ statistics.total_taxes }} {{ __(currency) }}</p>
            </li>
            <li>
              <span>{{ __("Total Cost") }}</span>
              <p>{{ statistics.total_cost }} {{ __(currency) }}</p>
            </li>
            <li>
              <span>{{ __("Total Receipts") }}</span>
              <p>{{ statistics.total_receipts }} {{ __(currency) }}</p>
            </li>
            <li>
              <span>{{ __("Balance") }}</span>
              <p>{{ statistics.the_total_credit }} {{ __(currency) }}</p>
            </li>
            <li>
              <span>{{ __("Total Debtor") }}</span>
              <p class="totalDebtor">
                {{ Math.abs(statistics.total_debtor) }} {{ __(currency) }}
              </p>
            </li>
            <li>
              <span>{{ __("Total Creditor") }}</span>
              <p class="totalCreditor">
                {{ statistics.total_creditor }} {{ __(currency) }}
              </p>
            </li>
          </ul>
          <hr /> -->
              ...
              <tbody>
                <template v-if="collection.length">
                  ...
                  <tr v-for="(reservation, i) in collection" :key="i">
                    ...
                    <td>
                        <div class="service_tag_container">
                            <span class="service_tag" v-for="service in reservation.services_included">
                                {{ locale == 'en' ? service.service.name_en : service.service.name_ar}}
                            </span>
                        </div>
                    </td>
    ...
    <form
      ...
      method="post"
      style="display: none"
      action="/home/print/reservations-services-included-print-report"
    >
      <input
        type="hidden"
...
export default {
  ...
  methods: {
    ...
    getReservations(page = 1) {
      let url = `/nova-vendor/calender/reservations-data-have-services?page=${page}
  ...
  mounted() {
    this.crumbs = [
    {
            ...
            {
                text : 'Reservations with services included report',
                to : '#'
            }
    ];
    this.getReservations();
