<template>
    <div>
        ...
        <div id="deposit_management_page">
            <div class="title">{{ __('Customer movement report') }}</div>
            <div class="content_page">
                ...
                <div class="filter_area">
                    ...
                    <div class="item">
                        <input
                            type="text"
                            v-model.lazy="customerPhoneNumber"
                ...
                <div class="action_buttons">
                        ...
                        <p>{{__('Total Customers Count')}}  : {{paginator.total}}</p>
                    ...
                    <div class="table-responsive relative">
                        ...
                        >
                            ...
                            <template v-if="data.length">
                                ...
                                <tr v-for="(customer,i) in data" :key="i">
                                    <td>{{customer.name}}</td>
                                    <td v-if="customer.highlight">
                                        <label v-if="customer.highlight.name" class="customer-label" :style="{ borderColor: customer.highlight.color , background : customer.highlight.color+'50' }" >{{customer.highlight.name[locale]}}</label>
                                    </td>
                                    <td v-else>-</td>
                                    <td>{{customer.id_number}}</td>
                                    <td><div class="phone_num">{{customer.phone}}</div></td>
                                    <td>{{__(customer.gender)}}</td>
                                    <td>{{customer.reservations_count}}</td>
                                    <td v-if="customer.nationality">
                                        <label v-if="customer.nationality.title">{{customer.nationality.title[locale]}}</label>
                                    </td>
                                    <td v-else>-</td>
                                </tr>
                            </template>
                            <template v-else>
                                <tr>
                                    <td colspan="11">{{__('No Customers Found!')}}</td>
                                </tr>
                            </template>
                            </tbody>
                        </table>
        ...
        <form id="customers_form" target="_blank" method="post"  style="display: none" action="/home/print/customersReport">
...
export default {
    ...
    methods:{
        ...
        excelExport(){
            ...
            axios.get(window.FANDAQAH_API_URL + '/customers/excel' , config).then(response => {
        ...
        getData(){
            ...
            axios.get(window.FANDAQAH_API_URL + `/customers/list` , config)
                .then((response) => {
    ...
    watch:{
        customerIdNumber: function (val) {
            if(val != null){
                let opt = {}
                opt["filter[by_id_number]"] = val;
                opt["page"] = 1;
                this.$router.push({
                    name : 'customers-report',
        ...
        customerPhoneNumber: function (val) {
            if(val != null){
                let opt = {}
                opt["filter[by_phone_number]"] = val;
                opt["page"] = 1;
                this.$router.push({
                    name : 'customers-report',
                    query: Object.assign({}, this.$route.query, opt)
