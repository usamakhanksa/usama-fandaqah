<template>
    <div>
        ...
        <div id="deposit_management_page">
            <div class="title">{{ __('Reservation Resources Report') }}</div>
            <div class="content_page">
                ...
                <div class="filter_area">
                    ...
                    <div class="item" v-show="sources.length">
                        <select v-model="sourceId">
                            <option value="" selected>{{__('Source')}}</option>
                            <option v-for="(source,i) in sources" :value="source.id" :key="i">{{  source.name[locale] }}</option>
                        </select>
                    </div>
                    ...
                    <div class="item">
                        <input
                            type="text"
                            v-model.lazy="reservationSourceNumber"
                        ...
                        >
                            ...
                            <template v-if="data.length">
                                <tr v-for="(obj,index) in data" :key="index">
                                    ...
                                    <td v-if="obj.customer && !obj.company"><router-link :to="{path : `/resources/customers/${obj.customer.id}`}" class="text-info-dark">{{obj.customer.name}}</router-link></td>
                                    ...
                                    <td v-else>-</td>
                                    <td>{{obj.source ? obj.source.name : '-'}}</td>
                                    <td>{{obj.source_number ? obj.source_number : '-'}}</td>
                                    <td>
                                        <p class='text-success' v-if="obj.status === 'confirmed'">{{__('confirmed')}}</p>
                                        <p class='text-danger' v-else-if="obj.status === 'canceled'">{{__('canceled')}}</p>
                                        <p class='text-timeout' v-else>{{__('timeout')}}</p>
                                    </td>
                                    <td>
                                        <p class='text-info' v-if="!obj.checked_in && !obj.checked_out">{{__('Pending')}}</p>
                                        <p class='text-success' v-else-if="obj.checked_in && !obj.checked_out">{{__('Checked In')}}</p>
                                        <p class='text-danger' v-else>{{__('Checked Out')}}</p>
                                    </td>
                                    <td>{{obj.date_in}}</td>
                                    <td>{{obj.date_out}}</td>
                                    <td>{{obj.nights}}</td>
                                    <td>{{obj.rent_type == 1 ? __('Daily') : __('Monthly')}}</td>
                            ...
                            <template v-else>
                                <tr>
                                    <td colspan="11" class="text-center">{{__('No Reservations Found')}}</td>
                                </tr>
                            </template>
        ...
        <!-- Single Print  -->
        <a :href="'/home/print/reservationResources' + current_print_url + `&total=${totals.total_cost}`" target="_blank" hidden ref="print_link"></a>
    </div>
</template>
...
export default {
    ...
    methods:{
        ...
        excelExport(){
            ...
            axios.get(`/nova-vendor/transactions-feature/reservationResourceExcel`, config)
            .then(response => {
        ...
        printReport(){
            // $('#reservation_sources_form').submit();
        ...
        getTotals() {
            ...
            axios.get(`/nova-vendor/transactions-feature/reservationResourcesTotals`, config)
            .then((response) => {
        ...
        getData(){
            ...
            axios.get(`/nova-vendor/transactions-feature/reservationResources`, config)
                .then((response) => {
        ...
        getReservationsSources(){
            axios.get(window.FANDAQAH_API_URL + `/reservation-source/dropDown?team_id=${this.team_id}`)
                .then((response) => {
    ...
    watch:{
        ...
        reservationSourceNumber: function (val) {
            if(val != null){
                let opt = {}
                opt["filter[by_source_number]"] = val;
                opt["page"] = 1;
                this.$router.push({
                    name : 'reservation-sources-report',
                    query: Object.assign({}, this.$route.query, opt)
    ...
    mounted() {
        ...
        this.crumbs = [
            ...
            {
                text : 'Reservation Resources Report',
                to : '#'
            }
        ];
        this.query = this.$route.query;
