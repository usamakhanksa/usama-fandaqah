<template>
    <div>
        ...
        <div id="deposit_management_page">
            <div class="title">{{ __('Maintenance movement report') }}</div>
            <div class="content_page">
                ...
                <div class="filter_area">
                    ...
                    <div class="item" v-show="maintenanceActionTypes.length">
                        <select v-model="maintenanceActionTypeId">
                            <option value="" selected>{{__('Maintenance type')}}</option>
                        ...
                        >
                            ...
                            <template v-if="data.length">
                                <tr v-for="(obj,index) in data" :key="index">
                                    ...
                                    <td>
                                        {{ obj.completed_at ? __('completed') : __('pending') }}
                                    </td>
                                    ...
                                    <td v-else>-</td>
                                    <td v-if="obj.action_id">{{locale == 'en' ? obj.action.name_en : obj.action.name_ar}}</td>
                                    <td v-else>-</td>
                                    <td v-if="obj.expected_at">{{obj.expected_at}}</td>
                                    <td v-else>-</td>
                                    <td>{{obj.note ? obj.note : '-'}}</td>
                                    <td class="td-fit">
                                        <router-link  :to="`/resources/unit-maintenances/${obj.id}`" :title="__('View')">
...
export default {
    ...
    methods:{
        ...
        getData(){
            ...
            axios.get(window.FANDAQAH_API_URL + `/units/units-maintenance` , config)
                .then((response) => {
    ...
    watch:{
        ...
        maintenanceActionTypeId: function (val) {
            if(val != null){
                let opt = {}
                opt["filter[by_action]"] = val;
                opt["page"] = 1;
                this.$router.push({
                    name : 'units-maintenance-report',
                    query: Object.assign({}, this.$route.query, opt)
        ...
        statusFilter: function (val) {
            let opt = {};
            if (val === 'completed') {
                opt["filter[by_status]"] = 'completed';
            } else if (val === 'pending') {
                opt["filter[by_status]"] = 'pending';
            } else {
                delete opt["filter[by_status]"];
            }
            opt["page"] = 1;
    ...
    mounted() {
        ...
        this.crumbs = [
            ...
            {
                text : 'Maintenance movement report',
                to : '#'
            }
        ];
        this.query = this.$route.query;
