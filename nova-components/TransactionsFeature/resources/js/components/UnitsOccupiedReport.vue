<template>
    <div>
        ...
        <div id="deposit_management_page">
            <div class="title">{{ __('Occupieds report') }}</div>
            <div class="content_page">
                ...
                <div class="bg-white rounded w-full flex flex-wrap mb-5">
                    <div class="sm:w-1/1 md:w-1/2 lg:w-1/2 xl:w-1/2 p-4 reservations_cost">
                        <h2 class="inline-block capitalize text-lg w-full">{{__('Total occupancy rate')}} : <h4 class="inline-block font-normal mr-1"><b class="text-black-500">{{ total_occupied_percentage }} %</b></h4></h2>
                    </div>
                </div>
        ...
        <form id="units_occupied_report" target="_blank" method="post"  style="display: none" action="/home/print/unitsOccupiedReportPrint">
...
export default {
    ...
    methods:{
        ...
        getData(){
            ...
            axios.get(window.FANDAQAH_API_URL + `/units/units-occupied` , config)
                .then((response) => {
                    ...
                    this.total_occupied_percentage = response.data.meta.total_occupied_percentage ? (response.data.meta.total_occupied_percentage / response.data.meta.total).toFixed(2) : 0.00;
    ...
    mounted() {
        ...
        this.$nextTick(() => {
            this.flatpickrFrom = flatpickr(this.$refs.datePickerFrom, {
                ...
                mode: 'single',
                time_24hr: false,
                onReady() { self.$refs.datePickerFrom.parentNode.classList.add('date-filter')},
                onChange(){ self.dateFrom = self.$refs.datePickerFrom.value;},
                locale : self.locale
            })
            this.flatpickrTo = flatpickr(this.$refs.datePickerTo, {
                ...
                mode: 'single',
                time_24hr: false,
                onReady() { self.$refs.datePickerTo.parentNode.classList.add('date-filter')},
                onChange(){ self.dateTo = self.$refs.datePickerTo.value;},
                locale : self.locale
            })
        });
