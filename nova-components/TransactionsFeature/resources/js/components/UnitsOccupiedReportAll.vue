<template>
    <div>
        ...
        <div id="deposit_management_page">
            <div class="title">{{ __('Occupieds report for all hotels associated to this user') }}</div>
            <div class="content_page p-5">
                <div class="totals_area flex flex-row justify-between mb-4">
                    <div class="total_item">

                        <strong>{{ __('Units Count') }}:&nbsp </strong> {{ totalUnitsCount }}
                    </div>
                    <div class="total_item">
                        <span class="status-square available"></span>
                        <strong>{{ __('Available Units') }}:&nbsp </strong> {{ totalAvailableUnits }}
                    </div>
                    <div class="total_item">
                        <span class="status-square booked"></span>
                        <strong>{{ __('Booked Units') }}:&nbsp </strong> {{ totalBooked }}
                    </div>
                    <div class="total_item">
                        <span class="status-square cleaning"></span>
                        <strong>{{ __('Under Cleaning') }}:&nbsp </strong> {{ totalCleaning }}
                    </div>
                    <div class="total_item">
                        <span class="status-square maintenance"></span>
                        <strong>{{ __('Under Maintenance') }}:&nbsp </strong> {{ totalMaintenance }}
                    </div>
                </div>
                <!-- Table Listing Area -->
                ...
                <div class="table_area">
                    <div class="table-responsive relative">
                        <loading :active="isLoading" :loader="'spinner'" :color="'#7e7d7f'" :opacity="0.7" :height="20"
                            :width="20" :is-full-page="false"></loading>
                        <table class="table w-full" cellpadding="0" cellspacing="0">
                            <thead>
                                <tr>
                                    ...
                                    <th>{{ __('Available Units') }}</th>
                                    <th>{{ __('Booked Units') }}</th>
                                    <th>{{ __('Under Cleaning') }}</th>
                                    <th>{{ __('Under Maintenance') }}</th>
                            ...
                            <tbody>
                                <template v-if="data.length">
                                    <tr v-for="(obj, index) in data" :key="index">
                                        ...
                                        <td>{{ obj.team_id }}</td>
                                        <td>{{ obj.unit_count }}</td>
                                        <td>{{ obj.available_units }}</td>
                                        <td>{{ obj.booked }}</td>
                                        <td>{{ obj.cleaning }}</td>
                                        <td>{{ obj.maintance }}</td>
...
export default {
    ...
    computed: {
        totalUnitsCount() {
            return this.data.reduce((total, item) => total + item.unit_count, 0);
        },
        totalAvailableUnits() {
            return this.data.reduce((total, item) => total + item.available_units, 0);
        },
        totalCleaning() {
            return this.data.reduce((total, item) => total + item.cleaning, 0);
        },
        totalMaintenance() {
            return this.data.reduce((total, item) => total + item.maintance, 0);
        },
        totalOccupied() {
            return this.data.reduce((total, item) => total + item.occupied, 0);
        },
        totalBooked() {
            return this.data.reduce((total, item) => total + item.booked, 0);
        },
        dateFormat() {
            return 'Y-m-d'
        },
    ...
    methods: {
        ...
        getData() {
            ...
            axios.get( '/nova-vendor/transactions-feature/report/units-occupied-all', config)
                .then((response) => {
...
.status-square.available {
    background-color: green;
}

.status-square.cleaning {
    background-color: orange;
}

.status-square.maintenance {
    background-color: grey;
}
.status-square.booked {
    background-color: rgb(24, 24, 248);
}
</style>
