<template>
    <div>
        ...
        <div id="deposit_management_page">
            <div class="title">{{ __('Services Report') }}</div>
                ...
                <div class="filter_area">
                    ...
                    <div class="item">
                        <select v-model="loadedType">
                            <option value="" selected="selected">{{__('Type')}}</option>
                            <option :value="'on_reservations'">{{__('Loaded On Reservation')}}</option>
                            <option :value="'on_revenue'">{{__('Loaded On Services Revenue')}}</option>
                        </select>
                    </div><!-- item -->
                    ...
                    <div class="item" v-show="service_categories.length">
                        <select v-model="serviceCategory">
                            <option value="" selected>{{__('Service Type')}}</option>
                            <option v-for="(category,i) in service_categories" :value="category.name[locale]" :key="i">{{ category.name[locale] }}</option>
                            <option value="various_services">{{__('Various services')}}</option>
                        </select>
                    </div>
                  ...
                  <div class="statistics_area relative">
                    ...
                    <ul>
                        ...
                        <li>
                            ...
                            <p class="d-flex">{{calculations.total_ttx}} <span v-if="(currency && currency == 'SAR') || !currency" class="icon-saudi_riyal"></span> <span v-else>{{ __(currency) }}</span></p>
                        </li>

                        <li>
                            <span>{{__('Total Sum')}}</span>
                            <p class="d-flex">{{calculations.total_sum}} <span v-if="(currency && currency == 'SAR') || !currency" class="icon-saudi_riyal"></span> <span v-else>{{ __(currency) }}</span></p>
                        </li>
                        <li>
                            <span></span>
                            <p></p>
                    ...
                    <div class="table-responsive relative">
                        ...
                        >
                            ...
                            <tr>
                                ...
                                <th>{{__('Employee')}}</th>
                                <th v-permission = "'print pos transactions'">{{__('Actions')}}</th>
                            </tr>
                            </thead>
                            <tbody>
                            ...
                            <tr v-for="(obj,index) in data" :key="index">
                                ...
                                <td v-if="obj.payable_type == 'App\\Reservation'">
                                    <router-link v-if="!obj.payable.deleted_at && obj.payable.customer" :to="{name : 'reservation' , params : {id :  obj.payable.id }}" class="text-info-dark">{{obj.payable.number}}</router-link>
                                    <router-link v-else-if="!obj.payable.deleted_at && !obj.payable.customer" :to="{name : 'reservation-noc' , params : {id :  obj.payable.id }}" class="text-info-dark">{{obj.payable.number}}</router-link>
                                    <p v-else>{{obj.payable.number}}</p>
                                </td>
                                <td v-else>-</td>
                                <td v-if="obj.payable_type == 'App\\Reservation' &&  obj.payable">{{ obj.payable.unit_number + ' - ' + obj.payable.unit_name[locale] }}</td>
                                <td v-else>-</td>
                                <td>{{ obj.sub_total }} </td>
                                <td>{{ obj.vat_total }} </td>
                                <td>{{ obj.ttx_total }} </td>
                                <td>{{ obj.total_with_taxes }} </td>
                                <td>{{ obj.payable_type == 'App\\Reservation' ?  obj.received_from : __('Loaded On Services Revenue') }}</td>
                                <td v-if="obj.for == 'various_services'">{{ __('Various services') }}</td>
                                <td v-else>{{ obj.for }}</td>
                                <td>{{obj.transaction_date}}</td>
                                <td v-if="obj.payable_type == 'App\\Team' && obj.pay_later && obj.type == 'withdraw'">{{__("No Payment")}}</td>
                                <td v-if="obj.payable_type == 'App\\Reservation'">-</td>
                                <td v-if="obj.payable_type == 'App\\Team' && obj.type == 'deposit'">{{ __(capitalize(obj.payment_method)) }}</td>
                                <td>{{ obj.employee }}</td>
                                <td v-permission = "'print pos transactions'" class="td-fit">
                                        <button  :title="__('Print Transaction')" @click="setTargetTransaction(obj.hash_id)"  target="_blank">
                            ...
                            <template v-else>
                                <tr>
                                    <td colspan="13" class="text-center">{{__('No Service Transactions found')}}</td>
                                </tr>
                            </template>
        ...
        <!-- Single Print  -->
        <a class="display:none;"  :href="'/home/reservation/sub-invoice-service/' + hash_id  " ref="print" target="_blank"></a>
        <form id="services_form" target="_blank" method="post"  style="display: none" action="/home/print/servicesTransactionsReportPrint?from=sreport">
...
export default {
    ...
    methods:{
        ...
        excelExport(){
            ...
            axios.get(window.FANDAQAH_API_URL + '/services/excel-services?from=sreport' , config).then(response => {
        ...
        getData(){
            ...
            this.isLoading = true;
            axios.get(window.FANDAQAH_API_URL + `/services-logs/list?from=sreport` , config)
                .then((response) => {
        ...
        getTotals(){
            ...
            axios.get(window.FANDAQAH_API_URL + `/services-logs/get-totals` , config)
            .then(response => {
        ...
        getServiceCategories(){
            axios.get(window.FANDAQAH_API_URL + `/services-category/dropDown?team_id=${this.team_id}`)
                .then((response) => {
