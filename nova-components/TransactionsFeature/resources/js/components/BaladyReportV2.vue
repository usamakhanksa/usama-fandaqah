                ...
                <div class="filter_area">
                    <div class="item">
                        <select @change="getYear($event)" v-model="year">
                            ...
                            <option  v-for="(y,index) in fetchYears" :value="y" :key="index" >{{y}}</option>
                        </select>
                    </div><!-- item -->
                    ...
                    <div class="item">
                        <select @change="getMonth($event)" v-model="month">
                            ...
                            <option v-for="(m,index) in this.fetchMonths"  :value="m" :key="index">{{months[index]}}</option>
                        </select>
                    </div><!-- item -->
                    <!-- Reset Button -->
                        ...
                        <table>
                            ...
                            <template v-if="data.length">
                                <tr v-for="(reservation,index) in data" :key="index">
                                    ...
                                    <td>{{ reservation.ucname ? reservation.ucname : '-' }}</td>
                                    <td>-</td>
                                </tr>
                            </template>
                            <template v-else>
        ...
        methods:{
            ...
            getData() {
                ...
                const axiosInstance = axios.create({
                    baseURL: window.FANDAQAH_API_V2_URL,
                    headers: {
                ...
                axiosInstance.post(`/reports/balady`, {
                ...
                .then((response) => {
                    if(response.data.success){
                        ...
                        this.all_data = response.data.extras.all_data;
                        this.excel_data = response.data.extras.excel_data;
                        this.query = this.$route.query;
                        this.total = response.data.extras.total;
                    }else{
                        this.data = [];
                        this.paginator = {};
                        this.ids = [];
            ...
            excelExport () {
                    var month_title = Nova.app.__(Intl.DateTimeFormat('en', { month: 'long' }).format(new Date(this.month)));
                    var report_title = Nova.app.__('Balady Report') + ' ' + month_title +  ' ' + this.year;
                    this.isLoading = false;
                    let Ws = XLSX.utils.json_to_sheet(this.excel_data)
                    let wb = XLSX.utils.book_new();
                    XLSX.utils.book_append_sheet(wb, Ws, report_title );
                    XLSX.writeFile(wb,  report_title + '.xlsx');
                    this.$toasted.show( Nova.app.__('Data exported successfully'), {type: 'success'});
            },
            getMonth(event){
            ...
            generateXlsFileName(){
                return moment().format('[Balady-Report-]MMMDDYYYY[.xls]');
            },
        },
        watch:{
            month: function (val) {
