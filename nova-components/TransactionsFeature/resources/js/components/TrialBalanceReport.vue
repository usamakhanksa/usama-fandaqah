<template>
    <div>
        ...
        <div id="deposit_management_page">
            <div class="title">{{ __('Trial Balance Report') }}</div>
                ...
                <!-- Table Listing Area -->
                    ...
                    <div class="table_area"  v-if="data">
                        <div class="table-responsive relative">
                            ...
                            >
                                <thead>
                                    <tr>
                                        <th>{{__('Reservations Revenue')        }}</th>
                                        <th>{{__('Services Reveunue')           }}</th>
                                        <th>{{__('Total Revenue')               }}</th>
                                        <th>{{__('Ewa')                         }}</th>
                                        <th>{{__('Vat On Reservations')         }}</th>
                                        <th>{{__('Vat On Services')             }}</th>
                                        <th>{{__('Total Non Revenue')           }}</th>
                                        <th>{{__('Cash')                        }}</th>
                                        <th>{{__('Bank Transfer')               }}</th>
                                        <th>{{__('Mada')                        }}</th>
                                        <th>{{__('Credit')                      }}</th>
                                        <th>{{__('Credit Payment')              }}</th>
                                ...
                                <tbody>
                                    <template v-if="data">
                                        <tr>
                                            <td>{{ data.reservations_revenue_total.toFixed(2)       }}</td>
                                            <td>{{ data.services_revenue_total.toFixed(2)           }}</td>
                                            <td>{{ data.total_rsv_srv_revenue.toFixed(2)            }}</td>
                                            <td>{{ data.reservations_ewa_total.toFixed(2)           }}</td>
                                            <td>{{ data.reservations_vat_total.toFixed(2)           }}</td>
                                            <td>{{ data.services_vat_total.toFixed(2)               }}</td>
                                            <td>{{ data.total_non_revenue.toFixed(2)                }}</td>
                                            <td>-{{ data.payment_type_cash.toFixed(2)               }}</td>
                                            <td>-{{ data.payment_type_bank_transfer.toFixed(2)      }}</td>
                                            <td>-{{ data.payment_type_mada.toFixed(2)               }}</td>
                                            <td>-{{ data.payment_type_credit.toFixed(2)             }}</td>
                                            <td>-{{ data.payment_type_credit_payment.toFixed(2)     }}</td>
                                            <td>-{{ data.payment_sum_all_types.toFixed(2)           }}</td>
                                            <td>{{ data.total_transaction.toFixed(2)                }}</td>
                                        </tr>
                                    </template>
                                </tbody>
                            </table>
                    ...
                    <div v-else class="table_area">
                        ...
                        <div class="no_data_show">
                            ...
                            <span>{{__('Please start to select month and year to initiate trial balance report')}}</span>
                        </div><!-- no_data_show -->
                    </div>
            </div>
        </div>
        ...
        <!-- Single Print  -->
        <form id="invoices_form" target="_blank" method="post"  style="display: none" action="/home/print/trialBalanceReportPrint">
...
export default {
    ...
    methods:{
        ...
        async excelExport(){
            try {
                this.isLoading = true;
                const response = await axios.post(window.FANDAQAH_API_URL + '/reports/trial-balance-excel' , {
                    data: this.data,
                }, {
                    headers: {
                        lang: this.locale
        ...
        async getData(){
            try {
                ...
                const response = await axios.get(window.FANDAQAH_API_URL + `/reports/trial-balance`, config);
