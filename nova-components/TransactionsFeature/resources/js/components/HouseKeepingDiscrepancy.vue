<template>
    <div>
        ...
        <div id="deposit_management_page">
            <div class="title">{{ __('HouseKeeping Discrepancies') }}</div>
                        ...
                        >
                            ...
                            <template v-if="data">
                                <tr v-for="(obj,index) in data" :key="index">
                                   ...
                                   <td>{{ getUnitName(obj.unit_name)                }}</td>
                                   <td>{{ getRoomType(obj.category_name)            }}</td>
                                   <td>{{ getStatus(obj.status)                     }}</td>
                                   <td>{{ getOccupationStatus(obj.occupied_total)   }}</td>
                                   <td></td>
                                </tr>
                            </template>
                            <template v-else>
        ...
        <!-- Single Print  -->
        <form id="invoices_form" target="_blank" method="post"  style="display: none" action="/home/print/houseKeepingDiscrepanciesReportPrint">
...
export default {
    name: "house-keeping-discrepancy",
    components:{
        Loading,
        Pagination
    },
    ...
    methods:{
        ...
        excelExport(){
            ...
            axios.post(window.FANDAQAH_API_URL + '/reports/housekeeping-discrepancy-excel' , {
                'data' : this.data ,
        ...
        getData(){
            ...
            axios.get(window.FANDAQAH_API_URL + `/reports/housekeeping-discrepancies` , config)
                .then((response) => {
        ...
        getRoomType (type) {
          let roomType = JSON.parse(type);
          return this.locale == 'ar' ? roomType.ar : roomType.en;
        },
        getUnitName (unit) {
            let unitName = JSON.parse(unit);
            return this.locale == 'ar' ? unitName.ar : unitName.en;
        }
    },
    watch:{
    ...
    mounted() {
        ...
        this.crumbs = [
            ...
            {
                text : 'HouseKeeping Discrepancies',
                to : '#'
            }
        ];
        this.query = this.$route.query;
