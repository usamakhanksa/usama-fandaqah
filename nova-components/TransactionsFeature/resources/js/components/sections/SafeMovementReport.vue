<template>
    <loading-view :loading="initialLoading">
        ...
        <div id="safe_movement_report_page">
            ...
            <div class="content_page">
                <div class="filter_area">
                    <div class="item">
                        <date-time-picker-from
                            ...
                            :placeholder="__('Select Date From')"
                        />
                    </div><!-- item -->
                    <div class="item">
                        <date-time-picker-to
                            :enable-seconds="false"
                            :twelve-hour-time="false"
                            :placeholder="__('Select Date To')"
                        />
                    </div><!-- item -->
                    <div class="item_button search">
                        <button
                    ...
                    <div v-show="isDateSelected" class="alert" role="alert">
                        <p>{{__('Search Process With Date Range')}}</p>
                        <span>{{__('Our search engine is ready to start searching for deposit & withdraw transactions according to the selected date range which is')}} {{__('From')}} : <b>{{dateFrom}}</b> {{__('To')}} : <b>{{dateTo}}</b></span>
                    </div><!-- alert -->
                </div><!-- filter_area -->
                ...
                <div class="statistics_area">
                    <loading :active.sync="isGeneralLoading" :can-cancel="true" :loader="'bars'" :color="'#007bff'" :is-full-page="fullPage"></loading>
                    <ul>
                        <li>
                            <span>{{__('Total Deposit Transactions')}}</span>
                            <p>{{ totalDepositAmount }} {{__(currency)}}</p>
                        </li>
                        <li>
                            <span>{{__('Total Withdraw Transactions')}}</span>
                            <p>{{Math.abs(totalWithdrawAmount)}} {{__(currency)}}</p>
                        </li>
                        <li>
                            <span>{{__('Safe Balance')}}</span>
                            <p>{{ (depositWithoutFormat + withdrawWithoutFormat).toFixed(2) }} {{__(currency)}}</p>
                        </li>

                        <li>
                            <span>{{__('Management Balance')}}</span>
                            <p v-if="">{{ ( Math.abs(toManagement) - Math.abs(fromManagement)).toFixed(2) }} {{__(currency)}}</p>
                        </li>
                ...
                <div class="bonds_statistics">
                    <div class="col">
                        ...
                        <div class="col_content">
                            <ul>
                                <li>{{__('Total Cash')}} <p class="deposit_price">{{depositStatistics.cash}}</p> {{__(currency)}}</li>
                                <li>{{__('Total Bank Transfer')}} <p class="deposit_price">{{depositStatistics.bank_transfer}}</p> {{__(currency)}}</li>
                                <li>{{__('Total Mada')}} <p class="deposit_price">{{depositStatistics.mada}}</p> {{__(currency)}}</li>
                                <li>{{__('Total Credit Card')}} <p class="deposit_price">{{depositStatistics.credit_card}}</p> {{__(currency)}}</li>
                                <li>{{__('Total Funds Transferred From Management')}} <p v-if="depositStatistics" class="deposit_price">{{(depositStatistics.total_funds_from_management).toFixed(2)}}</p>{{__(currency)}}</li>
                            </ul>
                        </div><!-- col_content -->
                    </div><!-- col -->
                    <div class="col">
                        ...
                        <div class="col_content">
                            <ul>
                                <li>{{__('Total Cash')}} <p class="withdraw_price">{{withdrawStatistics.cash}}</p> {{__(currency)}}</li>
                                <li>{{__('Total Bank Transfer')}} <p class="withdraw_price">{{withdrawStatistics.bank_transfer}}</p> {{__(currency)}}</li>
                                <li>{{__('Total Mada')}} <p class="withdraw_price">{{withdrawStatistics.mada}}</p> {{__(currency)}}</li>
                                <li>{{__('Total Credit Card')}} <p class="withdraw_price">{{withdrawStatistics.credit_card}}</p> {{__(currency)}}</li>
                                <li>{{__('Total Funds Transferred To Management')}} <p v-if="withdrawStatistics" class="withdraw_price">{{ Math.abs(withdrawStatistics.total_funds_to_management).toFixed(2)}}</p> {{__(currency)}}</li>
                            </ul>
                        </div><!-- col_content -->
                    </div><!-- col -->
                        ...
                        <div class="col_content">
                            <div class="table-responsive">
                                <table>
                                    ...
                                    <tbody>
                                    <tr v-if="depositResults.length" v-for="(depositTransaction , index) in depositResults" :key="index">
                                        <td>{{ depositTransaction.number }}</td>
                                        <td>{{ (Math.abs(Number(depositTransaction.amount/100))).toFixed(2) }}</td>
                                        <!-- <td>{{ depositTransaction.meta['from'] }}</td> -->
                                        <td>{{ __(capitalizeFirstLetter(depositTransaction.meta['payment_type'])) }}</td>
                                        <td>{{ depositTransaction.meta['statement'] }}</td>
                                        <td>{{ depositTransaction.meta['employee'] }}</td>
                                        <td>{{ depositTransaction.meta['date'] }}</td>
                                    </tr>
                                    <tr v-if="!depositResults.length">
                                        <td colspan="6">{{__('No Deposit Transactions Matches Your Criteria')}}</td>
                                    </tr>
                                    </tbody>
                                </table>
                            </div><!-- table-responsive -->
                            <div class="pagination_area" v-if="depositResults.length && depositPagination.totalPages > 20">
                                <pagination
                                    :page-count="depositPagination.lastPage"
                                    :page-range="3"
                                    :margin-pages="2"
                                    :click-handler="getDepositTransactions"
                        ...
                        <div class="col_content">
                            <div class="table-responsive">
                                <table>
                                    ...
                                    <tbody>
                                    <tr v-if="withdrawResults.length" v-for="(withdrawTransaction , index) in withdrawResults" :key="index">
                                        <td>{{ withdrawTransaction.number }}</td>
                                        <td>{{ (Math.abs(Number(withdrawTransaction.amount/100))).toFixed(2) }}</td>
                                        <!-- <td>{{ withdrawTransaction.meta['from'] }}</td> -->
                                        <td>{{ __(capitalizeFirstLetter(withdrawTransaction.meta['payment_type'])) }}</td>
                                        <td>{{ withdrawTransaction.meta['statement'] }}</td>
                                        <td>{{ withdrawTransaction.meta['employee'] }}</td>
                                        <td>{{ withdrawTransaction.meta['date'] }}</td>
                                    </tr>
                                    <tr v-if="!withdrawResults.length">
                                        <td colspan="6">{{__('No Withdraw Transactions Matches Your Criteria')}}</td>
                                    </tr>
                                    </tbody>
                                </table>
                            </div><!-- table-responsive -->
                            <div class="pagination_area" v-if="withdrawResults.length && withdrawPagination.totalPages > 20">
                                <pagination
                                    :page-count="withdrawPagination.lastPage"
                                    :page-range="3"
                                    :margin-pages="2"
                                    :click-handler="getWithdrawTransactions"
                                    :prev-text="__('Previous')"
                                    :next-text="__('Next')"
                                    :container-class="'pagination  w-full flex justify-center'"
                                    :page-class="'page-item'"
        ...
        <form id="safe_withdraw_report" target="_blank" method="post"  style="display: none" action="/home/print/safeMovementReport">
            ...
            <input type="hidden" :value="JSON.stringify(allWithdrawTransactions)" name="safeMovementData">
        </form>
        ...
        <form id="safe_deposit_report" target="_blank" method="post"  style="display: none" action="/home/print/safeMovementReport">
            ...
            <input type="hidden" :value="JSON.stringify(allDepositTransactions)" name="safeMovementData">
        </form>
        ...
        <form id="safe_deposit_report_all" target="_blank" method="post"  style="display: none" action="/home/print/safeMovementReportAll">
            ...
            <input type="hidden" :value="JSON.stringify(allDepositTransactions)" name="depositData">
            <input type="hidden" name="withdrawStatistics" :value="JSON.stringify(withdrawStatistics)">
            <input type="hidden" :value="JSON.stringify(allWithdrawTransactions)" name="withdrawData">
    ...
    export default {
        ...
        data() {
            return {
                ...
                actualCredit : 0 ,
                generalStatistics : {},
                withdrawWithoutFormat : null,
                depositWithoutFormat : null,
                toManagement : null ,
                fromManagement : null
            }
        },
        ...
        methods:{
            ...
            getDepositTransactions(dPagi=1){

                dPagi = '/nova-vendor/transactions-feature/depositTransactions?page='+ dPagi + '&dateFrom='+ this.dateFrom + '&dateTo=' + this.dateTo ;
                this.isDepositLoading = true ;
                Nova.request().get(dPagi)
                    .then((res)=>{

                        if(res.data.flag === 'data_found'){
                            this.depositResults = res.data.deposit_transactions.data ;
                            this.depositPagination = {
                                currentPage : res.data.deposit_transactions.current_page ,
                                lastPage : res.data.deposit_transactions.last_page ,
                                fromPage : res.data.deposit_transactions.from,
                                toPage : res.data.deposit_transactions.to,
                                totalPages : res.data.deposit_transactions.total,
                                pathPage : res.data.deposit_transactions.path + '?page=',
                                firstPageUrl : res.data.deposit_transactions.first_page_url ,
                                lastPageUrl : res.data.deposit_transactions.last_page_url ,
                                nextPageUrl : res.data.deposit_transactions.next_page_url ,
                                prevPageUrl : res.data.deposit_transactions.prev_page_url ,

                            };
                            this.depositStatistics = {
                                cash : res.data.cash ,
                                bank_transfer : res.data.bank_transfer ,
                                mada : res.data.mada ,
                                credit_card : res.data.credit_card ,
                                total_funds_from_management : res.data.total_funds_from_management

                            };

                            this.fromManagement = res.data.total_funds_from_management;
                            this.allDepositTransactions = res.data.all_deposit_transactions ;

                            this.totalDepositAmount = res.data.total_deposit;
                            this.isGeneralLoading = false ;

                            // push data to general statistics object
                            this.generalStatistics['totalDepositAmount'] =  this.totalDepositAmount ;
                            ...
                            this.generalStatistics['depositWithoutFormat'] = res.data.total_deposit_without_format
                            this.depositWithoutFormat =  res.data.total_deposit_without_format;
            ...
            getWithdrawTransactions(wPagi=1){
                wPagi = '/nova-vendor/transactions-feature/withdrawTransactions?page='+ wPagi + '&dateFrom='+ this.dateFrom + '&dateTo=' + this.dateTo ;
                this.isWithdrawLoading = true ;
                Nova.request().get(wPagi)
                    .then((res)=>{

                        if(res.data.flag === 'data_found'){
                            this.withdrawResults = res.data.withdraw_transactions.data ;
                            this.withdrawPagination = {
                                currentPage : res.data.withdraw_transactions.current_page ,
                                lastPage : res.data.withdraw_transactions.last_page ,
                                fromPage : res.data.withdraw_transactions.from,
                                toPage : res.data.withdraw_transactions.to,
                                totalPages : res.data.withdraw_transactions.total,
                                pathPage : res.data.withdraw_transactions.path + '?page=',
                                firstPageUrl : res.data.withdraw_transactions.first_page_url ,
                                lastPageUrl : res.data.withdraw_transactions.last_page_url ,
                                nextPageUrl : res.data.withdraw_transactions.next_page_url ,
                                prevPageUrl : res.data.withdraw_transactions.prev_page_url ,
                            ...
                            this.withdrawStatistics = {
                                ...
                                mada : res.data.mada ,
                                credit_card : res.data.credit_card ,
                                total_funds_to_management : res.data.total_funds_to_management,

                            }

                            this.toManagement = res.data.total_funds_to_management;


                            this.changeableTotalFundsToManagement = res.data.total_funds_to_management ;
                            this.fixedTotalFundsToManagement = res.data.fixed_total_funds_to_management ;


                            this.allWithdrawTransactions = res.data.all_withdraw_transactions ;

                            this.totalWithdrawAmount = res.data.total_withdraw_without_format ;

                            this.isGeneralLoading = false ;

                            // push data to general statistics object
                            // this.generalStatistics.push(this.changeableTotalFundsToManagement);
                            this.generalStatistics['changeableTotalFundsToManagement'] = this.changeableTotalFundsToManagement;
                            // this.generalStatistics.push(this.fixedTotalFundsToManagement);
                            this.generalStatistics['fixedTotalFundsToManagement'] = this.fixedTotalFundsToManagement;
                            // this.generalStatistics.push(this.totalWithdrawAmount);
                            this.generalStatistics['totalWithdrawAmount'] = this.totalWithdrawAmount

                            // this.withdrawWithoutFormat = this.total_withdraw_without_format;
                            this.generalStatistics['withdrawWithoutFormat'] = res.data.total_withdraw_without_format;
                            this.generalStatistics['toManagement'] = this.toManagement;
                            this.withdrawWithoutFormat = res.data.total_withdraw_without_format;
                        }
            ...
            // updateDate() {
            //
            //     this.dateFrom = this.selectedDate.start ;
            //     this.dateTo   = this.selectedDate.end ;
            //
            //     if(this.dateFrom && this.dateTo){
            //         this.dateFrom = moment(String(this.selectedDate.start)).format('YYYY-MM-DD');
            //         this.dateTo = moment(String(this.selectedDate.end)).format('YYYY-MM-DD');
            ...
            excelExport(type){
                ...
                switch (type) {
                    case 'deposit':
                        this.isDepositLoading = true ;
                        Nova.request().post('/nova-vendor/transactions-feature/safeMovementReportExcel', {
                            params: this.allDepositTransactions
                        }).then(response => {
                    ...
                    case 'withdraw':
                        this.isWithdrawLoading = true ;
                        Nova.request().post('/nova-vendor/transactions-feature/safeMovementReportExcel', {
                            params: this.allWithdrawTransactions
                        }).then(response => {
            ...
            //
            //         case 'deposit':
            //             self.isDepositLoading = true ;
            //             Nova.request().post('/nova-vendor/transactions-feature/safeMovementReportPrint', {
            //                 params: this.allDepositTransactions
            //             }).then(response => {
            //
            //
            //                     printJS({
            //                         documentTitle : self.__('Deposit Transactions') ,
            //                         header : self.__('Deposit Transactions') ,
            ...
            //                         showModal : true ,
            //                         modalMessage : self.__('Preparing the document') ,
            //                         printable: response.data.printable,
            //                         type: 'json',
            //                         properties: response.data.properties
            //                     });
            //
            //                 self.isDepositLoading = false ;
            //
            //
            //             });
            //             break;
            ...
            //
            //                 printJS({
            //                     documentTitle : self.__('Withdraw Transactions') ,
            //                     header : self.__('Withdraw Transactions') ,
            //                     headerStyle : 'font-weight: 300;' ,
            //                     css: ['https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css',
            //                         'https://fonts.googleapis.com/css?family=Open+Sans:400,700'
            //                     ],
        ...
        computed: {
            ...
            managementCreditOnPeriod(){
                return this.isDisabledSearchBtn
            },
        },
        watch:{
            dateValues: {
