<template>
  <loading-view :loading="initialLoading" :dusk="lens + '-lens-component'">
      <bread-crumb />
    <div id="reservation_resources_page">
        ...
        <div class="statistics_area">
          <ul>
            ...
            <li>
              ...
              <p>{{ totalReservationsAmount }} {{__(currency)}}</p>
            </li>
          </ul>
        </div><!-- statistics_area -->
        ...
        <div class="table_area">
          <loading-view :loading="loading">
            ...
            <div class="table_responsive">
              ...
              <form id="reservation_resources_form" target="_blank" method="post"  style="display: none" action="/home/print/reservationResources">
                <input type="hidden" :value="JSON.stringify(resourcesIds)" name="resourcesIds">
                <input type="hidden" :value="JSON.stringify(totalReservationsAmount)" name="totalReservationsAmount">
              </form>
            </div><!-- table_responsive -->
          </loading-view>
        </div><!-- table_area -->
      </div><!-- content_page -->
    </div><!-- reservation_resources_page -->
  </loading-view>
</template>
...
<script>
    ...
    //  import BigFilterCard from './filters/BigFilterCard'
    ...
    export default {
        ...
        data: () => ({
            ...
            orderByDirection: '',
            trashed: '',
            headingTitle : 'Reservation Resources' ,
        ...
        methods: {
            ...
            excelExport(){
                Nova.request().post('/nova-vendor/transactions-feature/reservationResourceExcel', {
                    params: this.resourcesIds
                }).then(response => {
                        ...
                        // fire success toast
                        this.$toasted.show(this.__('Reservation Resources Report was exported successfully'), {type: 'success'});
            ...
            // printReport(){
            //     Nova.request().post('/nova-vendor/transactions-feature/reservationResourcePrint', {
            //         params: this.resourcesIds
            //     }).then(response => {
            //
            //             printJS({
            ...
            // },
            printReport(){
                $('#reservation_resources_form').submit();
            },
            selectAllResources() {
        ...
        mounted() {
            Nova.$on('resources-loaded' , () => {
                ...
                Nova.request().post('/nova-vendor/transactions-feature/refreshData' , {filters : this.encodedFilters})
                    .then((response) => {
                        this.resourcesIds = response.data.reservations ;
                        this.totalReservations = response.data.reservations_count ;
                        this.totalReservationsAmount = response.data.reservations_total_amount ;
                    }).catch((error) => {
