<template>
  <loading-view :loading="initialLoading" :dusk="resourceName + '-index-component'">
      <bread-crumb />
    <div id="deposit_transactions_page">
      <custom-index-header v-if="!viaResource" :resource-name="resourceName" />
      <div class="title">{{__('List Deposit Transactions')}}</div>
      <div class="content_page">
          ...
          <ul>
            <li>
              <loading-view :loading="loading">
                ...
                <p>{{ depositStatistics.total_cash  }} {{__(currency)}}</p>
              </loading-view>
            </li>
            <li>
              <loading-view :loading="loading">
                <span>{{__('Total Bank Transfer')}}</span>
                <p>{{ depositStatistics.total_bank_transfer_cash  }} {{__(currency)}}</p>
              </loading-view>
            </li>
            <li>
              <loading-view :loading="loading">
                <span>{{__('Total Mada')}}</span>
                <p>{{ depositStatistics.total_mada  }} {{__(currency)}}</p>
              </loading-view>
            </li>
            <li>
              <loading-view :loading="loading">
                <span>{{__('Total Credit Card')}}</span>
                <p>{{ depositStatistics.total_credit  }} {{__(currency)}}</p>
              </loading-view>
            </li>
            <li>
              <loading-view :loading="loading">
                <span>{{__('Total')}}</span>
                <p>{{ depositStatistics.grand_total  }} {{__(currency)}}</p>
              </loading-view>
            </li>
          </ul>
        </div><!-- statistics_area -->
        ...
        <div class="action_buttons">
          <button type="button" v-show="depositTransactions.length" class="excel_button" @click="excelExport"></button>
          <button type="button" v-show="depositTransactions.length" class="print_button" @click="printReport"></button>
        </div><!-- action_buttons -->
        ...
        <div class="table_area">
          <loading-view :loading="loading">
            <div class="table_responsive">
              <transaction-table
                @deposit-transaction-type-changed="depositType($event)"
...
<script>
    ...
    import DepositTransactionType from "../helpers/DepositTransactionType";
    ...
    export default {
        ...
        methods: {
            ...
            resetFilters(){
                ...
                Nova.$emit('reset-clicked-deposit') ;
            ...
            //                 showModal : true ,
            //                 modalMessage : this.__('Preparing the document') ,
            ...
            pdfExport(){
                ...
                /**
                 * @author Emad Rashad
                 * Handling PDF Download from laravel to vue js
                 * @description the bug was in the response , the generated pdf is rendered in network tab request
                 * the most important thing about this new function is this param : responseType: 'arraybuffer',
                 * why this problem  :  all the community confirms that vue js and laravel ships through different ports
                 */
                Nova.request().post('/nova-vendor/transactions-feature/transactions/exportPdf' , {
                    transactions: this.depositTransactions,
                    type:'withdraw-deposit-transaction'
                        ...
                        link.download = self.__(filename)
                        link.click()
            ...
            excelExport () {
                Nova.request().get('/nova-vendor/transactions-feature/transactions/exportExcel' , {
                    ...
                    if(response.data.status == 'no_selection_to_export'){
                        ...
                        // Export file
                        XLSX.writeFile(wb, 'Deposit Transactions.xlsx' , {defaultCellStyle : defaultCellStyle})
                        // fire success toast
        ...
        mounted() {
            ...
            // console.log(Nova.app.$el.find('.view_reservation'));
            ...
            this.$on('resources-loaded' , () => {
                ...
                Nova.request().post('/nova-vendor/transactions-feature/allResourceStatistics' ,{ type:'deposit' , termId : this.termId , filters : this.encodedFilters})
                    .then((res)=>{
                        ...
                        this.depositTransactions = this.depositStatistics.transactions_query ;

                        this.encodedTransactionsStatistics = JSON.stringify(this.depositStatistics);
