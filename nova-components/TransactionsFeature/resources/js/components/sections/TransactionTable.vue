<template>
    ...
    >
        ...
        <tr>
            ...
            <!-- Field Names -->
            <th v-for="(field , index) in fields" :class="`text-${field.textAlign}`" :key="index">
                <sortable-icon
...
<script>
    ...
    export default {
        ...
        mounted() {
            ...
            // let fetched_resources = this.resources ;
            // let resources_ids = [] ;
            // fetched_resources.map(function(resource){
            //     resources_ids.push(resource.title) ;
            //     return resources_ids
            // });
            //
            // Nova.request().post('/nova-vendor/transactions-feature/numbers-components' , resources_ids).then(response => {
            //       Nova.$emit('cash_number' , response.data.cash) ;
            //       Nova.$emit('bank_cash_number' , response.data.other) ;
            //       Nova.$emit('grand_total' , response.data.grand) ;
            // })
        }
    }
</script>
