        ...
        <div class="filter_area">
            ...
            <div class="item_button reset">
                ...
                <div class="action_buttons" v-if="resources.length">
                    ...
                    <a onclick="$('#ewa_report_form').submit();" target="_blank">
                    ...
                    <form  id="ewa_report_form" target="_blank" method="post"  style="display: none" action="/home/print/baladyReport">
                        <input type="hidden" :value="JSON.stringify(all_data)" name="all_data">
                        <input type="hidden" :value="JSON.stringify(total_amount)" name="total_amount">

                    </form>
                </div><!-- action_buttons -->

                <div class="table_area">
                        <div class="title" v-if="resources.length">
                            <span>{{__('Reservations')}} <p>({{resourcesPaginator.total}})</p></span>
                            <span>{{__('Total Amount')}} <p>({{total_amount}} {{__(currency)}})</p></span>
                        </div><!-- title -->
                        <div class="table_responsive relative">
                            ...
                            <table>
                                ...
                                <template v-if="resources.length">
                                    <tr v-for="(reservation,index) in resources" :key="index">
                                    <td>{{ reservation.unumber }}</td>
                            ...
                            <div class="w-full flex flex-wrap mb-4 mt-3 justify-center" v-if="resources.length && resourcesPaginator.total > 25">
                                <pagination
                                    :page-count="resourcesPaginator.lastPage"
                                    :page-range="3"
                                    :margin-pages="2"
                                    :value="resourcesPaginator.currentPage"
                                    :click-handler="getResources"
                                    :prev-text="'<'"
                                    :next-text="'>'"
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            getResources(page = 1) {
                this.loading = true ;
                axios.get(`/nova-vendor/transactions-feature/balady-report?year=${this.year}&month=${this.month}&page=${page}`)
                .then((res) => {
                    this.resources = res.data.collection.data;
                    this.all_data = res.data.all_data;
                    this.total_amount = res.data.total_amount;
                    this.resourcesPaginator = {
                        currentPage : res.data.collection.current_page ,
                        lastPage : res.data.collection.last_page ,
                        from : res.data.collection.from,
                        to : res.data.collection.to,
                        total : res.data.collection.total,
                        pathPage : res.data.collection.path + '?page=',
                        firstPageUrl : res.data.collection.first_page_url ,
                        lastPageUrl : res.data.collection.last_page_url ,
                        nextPageUrl : res.data.collection.next_page_url ,
                        prevPageUrl : res.data.collection.prev_page_url ,
                    };
                    this.loading = false;
                })
            ...
            excelExport () {
                axios.post('/nova-vendor/transactions-feature/balady-report-excel' , this.all_data).then((response) => {
                    ...
                    XLSX.utils.book_append_sheet(wb, monthlyData , this.__('Balady Report'))
                    // Export file
                    XLSX.writeFile(wb, this.__('Balady Report') + '.xlsx')
                    // fire success toast
