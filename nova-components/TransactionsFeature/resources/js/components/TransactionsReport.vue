<template>
    <div>
        ...
        <div id="deposit_management_page">
            <div class="title">{{ dynamicType === 'deposit' ?  __('Deposits') : __('Withdraws') }}</div>
            <div class="content_page">
                ...
                <div class="filter_area">
                    ...
                    <div class="item">
                        <select v-model="paymentType">
                            ...
                            <option v-if="dynamicType == 'deposit'" value="credit-payment">{{__('Credit Payment')}}</option>
                ...
                <div class="statistics_area relative">
                    ...
                    <ul>
                        ...
                        <li v-if="dynamicType == 'deposit'">
                            <span>{{__('Total Credit Payment')}}</span>
                ...
                <div class="action_buttons">
                    ...
                    <add-modal :type="dynamicType" :terms="terms"/>
                    <div class="buttons_area" v-if="paginator && paginator.totalPages < 500">
                ...
                <div class="table_area">
                    <div class="table-responsive relative">
                        ...
                        >
                            ...
                            <tr>
                                ...
                                <th>{{ __('Employee') }}</th>
                                <th v-permission = "'print transactions'">{{__('Actions')}}</th>
                            </tr>
                            </thead>
                            <tbody>
                            <template v-if="transactions.length">
                                ...
                                <tr v-for="(obj,i) in transactions" :key="i">
                                   ...
                                   <td v-if="obj.transaction_payable_type === 'App\\Team'">-</td>
                                    ...
                                    <template v-if="dynamicType === 'withdraw' && obj.transaction_enable_tax_on_withdraw">
                                    ...
                                    <td v-permission = "'print transactions'" class="td-fit">

                                         <button @click="hashTransactionId(obj,i)"  target="_blank"  :title="__('Print Transaction')" >
                                        ...
                                        <a :href="'/home/reservation/sub-invoice/' + hash_id  " style="display:none;"  ref="reservation_href" target="_blank"></a>
                                        <a :href="'/home/team/sub-invoice/' +  hash_id  " style="display:none;" ref="team_href" target="_blank"></a>
                    ...
                    <!-- Pagination -->
                    <div class="w-full flex flex-wrap mt-3 justify-center" v-if="transactions.length">
                        <pagination
                    ...
                    <div class="Results_area" v-if="transactions.length">
                        <p>{{__('Results')}}  : {{__('From')}} ( {{paginator.fromPage}} ) - {{__('To')}}  ( {{paginator.toPage}} )</p>
                        <p>{{__('Total Transactions')}}  : {{paginator.totalPages}}</p>
                    </div><!-- Results_area -->
                </div>
...
export default {
    ...
    methods:{
        ...
        excelExport(){
            ...
            axios.get(window.FANDAQAH_API_URL + '/transactions/excel' , config).then(response => {
                ...
                let file_name = this.dynamicType == 'deposit' ? this.__('Deposit Report') : this.__('Withdraw Report')
        ...
        getTotals(){
            ...
            .then(response => {
                    ...
                    this.paginator = {
                        ...
                        fromPage : response.data.pagination.from,
                        toPage : response.data.pagination.to,
                        totalPages : response.data.pagination.total,
                        pathPage : response.data.pagination.path + '?page=',
                        firstPageUrl : response.data.pagination.first_page_url ,
                        lastPageUrl : response.data.pagination.last_page_url ,
                        nextPageUrl : response.data.pagination.next_page_url ,
                        prevPageUrl : response.data.pagination.prev_page_url
                    }
        ...
        getTerms(){
            ...
            axios.get(window.FANDAQAH_API_URL + `/terms/dropDown?team_id=${this.team_id}&type=${type}`)
            .then((response) => {
    ...
    watch:{
        ...
        customerIdNumber: {
            handler : function (val) {
                if(val != null){
                    let opt = {}
                    opt["filter[by_customer_id_number]"] = val;
                    opt["page"] = 1;
                    this.$router.push({
                        name : 'transactions-report',
                        query: Object.assign({}, this.$route.query, opt)
    ...
    mounted() {
        ...
        this.crumbs = [
            ...
            {
                text : this.dynamicType === 'deposit' ? 'Deposits' : 'Withdraws',
                to : '#'
            }
        ];
        this.getTerms();
