<template>
    <div>
        ...
        <div id="deposit_management_page">
            <div class="title">{{ __('Units Movement Report') }}</div>
                ...
                <div class="filter_area">
                    ...
                    <div class="item">
                        <input
                            ...
                            ref="reservationDate"
                            type="text"
                            :placeholder="__('Reservation Date')"
                        >
                    </div>
                        ...
                        >
                            ...
                            <template v-if="data.length">
                                <tr v-for="(obj,index) in data" :key="index">
                                    <td>{{ obj.unit ?  obj.unit.number + ' - ' + obj.unit.name[locale] : '-'}}</td>
                                    <td><router-link :to="{name : 'reservation' , params : {id : obj.id }}" class="text-info-dark">{{obj.reservation_number}}</router-link></td>
                                    <td>{{obj.date_in}}</td>
                                    <td>{{obj.date_out}}</td>
                                    <td>{{obj.created_at}}</td>
                                    <td>{{obj.customer ? obj.customer.name : '-'}} </td>
                                </tr>
                            </template>
                            <template v-else>
                                <tr>
...
export default {
    ...
    methods:{
        ...
        excelExport(){
            ...
            axios.get(window.FANDAQAH_API_URL + '/units/units-reservations-excel' , config).then(response => {
        ...
        getData(){
            ...
            axios.get(window.FANDAQAH_API_URL + `/units/units-reservations` , config)
                .then((response) => {
    ...
    mounted() {
        ...
        const self = this
        this.$nextTick(() => {
            this.flatpickrFrom = flatpickr(this.$refs.reservationDate, {
                enableTime: false,
                enableSeconds: false,
                disableMobile: "true",
                ...
                time_24hr: false,
                onReady() {
                    self.$refs.reservationDate.parentNode.classList.add('date-filter')
                },
                onChange(){
                    self.createdFrom = self.$refs.reservationDate.value;
                },
                locale : self.locale
            })
