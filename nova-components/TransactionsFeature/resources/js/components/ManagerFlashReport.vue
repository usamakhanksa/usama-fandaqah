<template>
    <div>
        ...
        <div id="deposit_management_page">
            <div class="title">{{ __('Manager Flash Report') }}</div>
            <div class="content_page">
                ...
                <div class="filter_area">
                    ...
                    <div class="item">
                        <select  v-model="dateFrom" ref="dateFrom">
                            <option :value="null">{{ __('Month') }}</option>
                            <option v-for="i in 12" :value="i < 10 ? `0${i}` : i">
                                {{ i < 10 ? `0${i}` : i }}
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="item">
                        <select  v-model="dateTo" ref="dateTo">
                            <option :value="null">{{ __('Year') }}</option>
                            <option v-for="i in getYears" :value="`${i}`">
                                {{ i }}
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="item flex items-center gap-2">
                        <input class="checkbox" type="checkbox" v-model="lastYearComparison" ref="lastYearComparison" />
                        <span>{{ __('Last year comparison') }}</span>
                    </div>
                ...
                <template v-if="data">
                    <div class="table_area" >
                        <div class="table-responsive relative" v-for="data in data.stats">
                            ...
                            >
                                <thead>
                                    <tr>
                                        <th colspan="46" v-if="data && data.month">{{ __(data.month) }} {{ data.year }}</th>
                                        <th colspan="46" v-else>
                                            -
                                        </th>
                                    </tr>
                                    <tr>
                                        <th>{{__('Total Rooms in Hotel')}}</th>
                                        <th>{{__('Rooms Occupied')}}</th>
                                        <th>{{__('Available Rooms')}}</th>
                                        <th>{{__('Out of Order Rooms')}}</th>
                                        <th>{{__('Out of Service Rooms')}}</th>
                                        <th>{{__('Day Use Rooms')}}</th>
                                        <th>{{__('Complimentary Rooms')}}</th>
                                        <th>{{__('Rooms Occupied minus Comp')}}</th>
                                        <th>{{__('Rooms Occupied minus House Use')}}</th>
                                        <th>{{__('Rooms Occupied minus Comp and House Use')}}</th>
                                        <th>{{ __('Total Rooms in Hotel minus OOO Rooms') }}</th>
                                        <th>{{__('Available Rooms minus OOO Rooms')}}</th>
                                        <th>{{ __('In-House Adults') }}</th>
                                        <th>{{ __('In-House Children') }}</th>
                                        <th>{{ __('Total In-House Persons') }}</th>
                                        <th>{{ __('Individual Persons In-House') }}</th>
                                        <th>{{ __('Individual Rooms In-House') }}</th>
                                        <th>{{ __('Company Rooms In-House') }}</th>
                                        <th>{{ __('% Rooms Occupied') }}</th>
                                        <th>{{ __('% Rooms Occupied minus Comp and House') }}</th>
                                        <th>{{ __('% Rooms Occupied minus Comp, House and OOO') }}</th>
                                        <th>{{ __('% Rooms Occupied minus Comp') }}</th>
                                        <th>{{ __('% Rooms Occupied minus House') }}</th>
                                        <th>{{ __('% Rooms Occupied minus Comp and OOO') }}</th>
                                        <th>{{ __('% Rooms Occupied minus House and OOO') }}</th>
                                        <th>{{ __('% Rooms Occupied minus OOO') }}</th>
                                        <th>{{ __('Arrival Rooms') }}</th>
                                        <th>{{ __('Arrival Persons') }}</th>
                                        <th>{{ __('Deducted Arrivals') }}</th>
                                        <th>{{ __('Departure Rooms') }}</th>
                                        <th>{{ __('Departure Persons') }}</th>
                                        <th>{{ __('Early Departure Rooms') }}</th>
                                        <th>{{ __('Early Departure Persons') }}</th>
                                        <th>{{ __('Individual Departure Rooms') }}</th>
                                        <th>{{ __('Individual Departure Persons') }}</th>
                                        <th>{{ __('Cancelled Reservations') }}</th>
                                        <th>{{ __('Reservations Made') }}</th>
                                        <th>{{ __('Clean Rooms') }}</th>
                                        <th>{{ __('Dirty Rooms') }}</th>
                                        <th>{{ __('ADR') }}</th>
                                        <th>{{ __('ADR minus Comp') }}</th>
                                        <th>{{ __('ADR minus House') }}</th>
                                        <th>{{ __('ADR minus Comp and House') }}</th>
                                        <th>{{ __('Room Revenue') }}</th>
                                        <th>{{ __('Service Revenue') }}</th>
                                        <th>{{ __('Payment') }}</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    <template v-if="data">
                                        <td>{{ data.total_rooms_in_property.toFixed(2) }}</td>
                                        <td>{{ data.total_rooms_occupied_in_property.toFixed(2) }}</td>
                                        <td>{{ data.total_available_rooms_in_property.toFixed(2) }}</td>
                                        <td>{{ data.out_of_order_rooms_in_property.toFixed(2) }}</td>
                                        <td>{{ data.out_of_service_rooms_in_property.toFixed(2) }}</td>
                                        <td>{{ data.day_use_rooms_in_property.toFixed(2) }}</td>
                                        <td>{{ data.complementary_rooms_in_property.toFixed(2) }}</td>
                                        <td>{{ data.rooms_occupied_minus_complementary.toFixed(2) }}</td>
                                        <td>{{ data.rooms_occupied_minus_house_use.toFixed(2) }}</td>
                                        <td>{{ data.rooms_occupied_minus_complementary_houseuse.toFixed(2) }}</td>
                                        <td>{{ data.total_rooms_minus_out_of_order.toFixed(2) }}</td>
                                        <td>{{ data.available_rooms_minus_out_of_order.toFixed(2) }}</td>
                                        <td>{{ data.adults_in_property.toFixed(2) }}</td>
                                        <td>{{ data.childs_in_property.toFixed(2) }}</td>
                                        <td>{{ data.total_persons_in_property.toFixed(2) }}</td>
                                        <td>{{ data.individual_persons_in_property.toFixed(2) }}</td>
                                        <td>{{ data.individual_rooms_in_house.toFixed(2) }}</td>
                                        <td>{{ data.company_room_in_house.toFixed(2) }}</td>
                                        <td>{{ data.percentage_room_occupied }}</td>
                                        <td>{{ data.percentage_rooms_occ_min_comp_house }}</td>
                                        <td>{{ data.percentage_rooms_occ_min_comp_house_ooo }}</td>
                                        <td>{{ data.percentage_rooms_occ_min_comp }}</td>
                                        <td>{{ data.percentage_rooms_occ_min_house }}</td>
                                        <td>{{ data.percentage_rooms_occ_min_comp_ooo }}</td>
                                        <td>{{ data.percentage_rooms_occ_min_house_ooo }}</td>
                                        <td>{{ data.percentage_rooms_occ_min_ooo }}</td>
                                        <td>{{ data.total_rooms_arrival.toFixed(2) }}</td>
                                        <td>{{ data.total_person_arrival.toFixed(2) }}</td>
                                        <td>{{ data.total_person_deducted.toFixed(2) }}</td>
                                        <td>{{ data.departure_rooms.toFixed(2) }}</td>
                                        <td>{{ data.departure_persons.toFixed(2) }}</td>
                                        <td>{{ data.early_departure_rooms.toFixed(2) }}</td>
                                        <td>{{ data.early_departure_persons.toFixed(2) }}</td>
                                        <td>{{ data.individual_departure_rooms.toFixed(2) }}</td>
                                        <td>{{ data.individual_departure_persons.toFixed(2) }}</td>
                                        <td>{{ data.cancelled_reservations.toFixed(2) }}</td>
                                        <td>{{ data.reservations_created.toFixed(2) }}</td>
                                        <td>{{ data.clean_rooms.toFixed(2) }}</td>
                                        <td>{{ data.dirty_rooms.toFixed(2) }}</td>
                                        <td>{{ data.adr.toFixed(2) }}</td>
                                        <td>{{ data.adr_minus_comp.toFixed(2) }}</td>
                                        <td>{{ data.adr_minus_house.toFixed(2) }}</td>
                                        <td>{{ data.adr_minus_comp_house.toFixed(2) }}</td>
                                        <td>{{ data.room_revenue.toFixed(2) }}</td>
                                        <td>{{ data.service_revenue.toFixed(2) }}</td>
                ...
                <template v-else>
                    <div class="table_area">
                        <div class="no_data_show">
                            ...
                            <span>{{__('Please start to select month and year to initiate manager flash report')}}</span>
                        </div><!-- no_data_show -->
                    </div>
                </template>
        ...
        <!-- Single Print  -->
        <form id="invoices_form" target="_blank" method="post"  style="display: none" action="/home/print/managerFlashReportPrint">
...
export default {
    ...
    methods:{
        ...
        async excelExport(){
            try {
                this.isLoading = true;
                const response = await axios.post(window.FANDAQAH_API_URL + '/reports/manager-flash-excel' , {
                    data: this.data,
                }, {
                    headers: {
                        lang: this.locale
        ...
        async getData(){
            try {
                this.data = null;
                this.isLoading = true;
                const response = await axios.post(window.FANDAQAH_API_URL + `/reports/manager-flash`, {
                    team_id: this.team_id,
                    year: this.dateTo,
                    month: this.dateFrom,
                    last_year_comparison: this.lastYearComparison
                });
        ...
        validateFields () {
            let isValid     = true;
            if(!this.dateTo) {
                this.$refs.dateTo.classList.add('error_field');
                isValid     = false;
            }
            if(!this.dateFrom) {
                this.$refs.dateFrom.classList.add('error_field');
                isValid     = false;
            }
            return isValid;
        }
    ...
    watch:{
        dateFrom: function (val) {
            this.$refs.dateFrom.classList.remove('error_field')
            if(this.validateFields()) {
                this.getData();
            }
        },
        dateTo: function (val) {
            this.$refs.dateTo.classList.remove('error_field')
            if(this.validateFields()) {
                this.getData();
            }
        },
        lastYearComparison: function (val) {
            if(this.validateFields()) {
                this.getData();
            }
        }
