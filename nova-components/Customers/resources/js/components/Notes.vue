<template>
       <div>
        ...
        <div id="notes_management" class="mb-8">
            <div class="title">{{ type == 'company' ? __('Notes on the company') : __('Notes to the customer') }}</div>
            <div class="content_page">


                <div class="table_title">
                    <span class="table" v-if="paginator">{{ type == 'company' ? __('Total notes on company') : __('Total notes on customer') }} <p>{{paginator.totalResults}}</p></span>
                    <create-note v-permission="'create companies notes'" v-if="type == 'company'" type="company"  :typeId="typeId" />
                    <create-note v-else  v-permission="'add notes'"  />
                </div><!-- table_title -->
                <!-- Table Listing Area -->
                    ...
                    <div class="table-responsive relative">
                        ...
                        >
                            ...
                            <template v-if="notes.length">
                                 <tr v-for="(obj,i) in notes" :key="i">
                                    <td v-if="type == 'company'">{{truncate(obj.body , 70 , '  ...')}}</td>
                                    <td v-else>{{truncate(obj.comment , 70 , '  ...')}}</td>
                                    <td v-if="type == 'company'">{{obj.creator.name}}</td>
                                    <td v-else>{{obj.commenter.name}}</td>
                                    <td>{{obj.created_at | formatDateWithTime}}</td>
                                    <td class="td-fit">
                                        <button @click="setTargetNote('show',obj)">
                                        ...
                                        <button v-if="type == 'company'" v-permission="'update companies notes'"  @click="setTargetNote('edit',obj)">
                                        ...
                                        <button v-else v-permission="'edit notes'" @click="setTargetNote('edit',obj)">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" aria-labelledby="edit" role="presentation" class="fill-current"><path d="M4.3 10.3l10-10a1 1 0 0 1 1.4 0l4 4a1 1 0 0 1 0 1.4l-10 10a1 1 0 0 1-.7.3H5a1 1 0 0 1-1-1v-4a1 1 0 0 1 .3-.7zM6 14h2.59l9-9L15 2.41l-9 9V14zm10-2a1 1 0 0 1 2 0v6a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2V4c0-1.1.9-2 2-2h6a1 1 0 1 1 0 2H2v14h14v-6z"></path></svg>
                                        </button>

                                        <button v-if="type == 'company'" v-permission="'delete companies notes'" @click="setTargetNote('delete',obj)">
                                        ...
                                        <button v-else v-permission="'delete notes'" @click="setTargetNote('delete',obj)">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" aria-labelledby="delete" role="presentation" class="fill-current"><path fill-rule="nonzero" d="M6 4V2a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2h5a1 1 0 0 1 0 2h-1v12a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V6H1a1 1 0 1 1 0-2h5zM4 6v12h12V6H4zm8-2V2H8v2h4zM8 8a1 1 0 0 1 1 1v6a1 1 0 0 1-2 0V9a1 1 0 0 1 1-1zm4 0a1 1 0 0 1 1 1v6a1 1 0 0 1-2 0V9a1 1 0 0 1 1-1z"></path></svg>
                                        </button>
                                    </td>
                                </tr>
                            ...
                            <template v-else>
                                <tr>
                                    <td colspan="13" class="text-center p-5">{{__('No Notes Found!')}}</td>
                                </tr>
                            </template>
                            </tbody>
                        </table>
                ...
                <div class="w-full flex flex-wrap mt-3 justify-center" >
                    <pagination
                        ...
                        :page-range="3"
                        :margin-pages="2"
                        :click-handler="type == 'company' ? getCompanyNotes : getNotes"
                ...
                <div class="Results_area" v-if="notes.length">
            ...
            <delete-confirm-component ref="deleteNote" :type="type" :note_id="targetNoteId" />
            <update-note :target_note="targetNote" :id="targetNoteId" :type="type" :typeId="typeId" ref="editNote" />
            <show-note :target_note="targetNote"  ref="showNote" />
        </div>
...
export default {
  ...
  methods: {
      setTargetNote(type,obj){
          this.targetNote = this.type == 'company' ? obj.body : obj.comment;
          if(type == 'show'){
              setTimeout(() => {
                  this.$refs.showNote.$refs.show.open();
              }, 0);
          }

          if(type == 'edit'){
              this.targetNoteId =  obj.id;
              setTimeout(() => {
                  this.$refs.editNote.$refs.edit.open();
              }, 0);
          }
          ...
          if(type == 'delete'){
              this.targetNoteId = obj.id;
              setTimeout(() => {
                  this.$refs.deleteNote.$refs.delete.open();
              }, 0);
          }
      },
      getNotes(page=1){
          this.isLoading = true;
          axios.get(`/nova-vendor/new/customers/notes/list?id=${this.$route.params.id}&page=${page}`)
          .then(response => {
      ...
      getCompanyNotes(page=1){
           this.isLoading = true;
          axios.get(`/nova-vendor/new/customers/companies/${this.typeId}/notes/list?page=${page}`)
          .then(response => {
              this.notes = response.data.data;
              this.paginator = {
                    currentPage : response.data.current_page ,
  ...
  mounted(){
      ...
      Nova.$on('call-customer-notes-query',() => {
          this.getNotes();
      });

      Nova.$on('call-companies-notes-query',() => {
        this.getCompanyNotes();
      });
  }
};
