<template>
  <div v-if="customer">
    ...
    <div class="flex w-full mb-4">
        <nav>
            <ul class="breadcrumbs">
                ...
                <li class="breadcrumbs__item">
                    <router-link :to="'#'">{{ this.customer ? customer.name : '' }}</router-link>
                </li>
            </ul>
        </nav>
    </div>
    ...
    <div class="mb-8 relative">
        <loading :active="isLoading" :loader="'spinner'" :color="'#7e7d7f'" :opacity="0.7" :height="20" :width="20" :is-full-page="false"></loading>
            <div class="flex items-center mb-3">
                <!-- Page Header  -->
                <h4 class="text-90 font-normal text-2xl flex-no-shrink">{{__('Customer Details')}}</h4>
                <div class="ml-3 w-full flex items-center">
                    <div class="flex w-full justify-end items-center"></div>
                    <!-- Edit Button  -->
                    <button v-permission="'manage customers'" class="btn btn-default btn-icon bg-primary" @click="openEditCustomerModal" :title="__('Edit')">
            ...
            <div class="card mb-6 py-3 px-6">
                <div class="flex border-b border-40">
                    <div class="w-1/4 py-4">
                        <h4 class="font-normal text-80">{{__('Name')}}</h4>
                    </div>
                    <div class="w-3/4 py-4">
                    <p class="text-90">{{customer.name}}</p>
                    </div>
                </div>
                <div class="flex border-b border-40">
                    <div class="w-1/4 py-4">
                        <h4 class="font-normal text-80">{{__('Customer Highlight')}}</h4>
                    </div>
                    <div class="w-3/4 py-4">
                        <div v-if="customer.highlight" v-html="customer.label"></div>
                        <p v-else>—</p>
                    </div>
                </div>
                <div class="flex border-b border-40">
                    ...
                    <div class="w-3/4 py-4">
                        <p v-if="customer.email" class="text-90">{{customer.email}}</p>
                        <p v-else>-</p>
                    </div>
                </div>
                <div class="flex border-b border-40">
                    ...
                    <div class="w-3/4 py-4">
                        <p class="text-90 phone-number-dir">{{customer.phone}}</p>
                    </div>
                </div>
                <div class="flex border-b border-40">
                    <div class="w-1/4 py-4">
                        <h4 class="font-normal text-80">{{__('Gender')}}</h4>
                    </div>
                    <div class="w-3/4 py-4">
                    <p v-if="customer.gender">{{ __(customer.gender) }}</p>
                    <p v-else>—</p>
                    </div>
                </div>
                <div class="flex border-b border-40">
                    <div class="w-1/4 py-4">
                        <h4 class="font-normal text-80">{{__('Customer Type')}}</h4>
                    </div>
                    <div class="w-3/4 py-4">
                        <p v-if="customer.customer_type">{{customer.customer_type_string}}</p>
                        <p v-else>-</p>
                    </div>
                </div>
                <div class="flex border-b border-40">
                    <div class="w-1/4 py-4">
                        <h4 class="font-normal text-80">{{__('Nationality')}}</h4>
                    </div>
                    <div class="w-3/4 py-4">
                        <p v-if="customer.nationality">{{customer.nationality_string}}</p>
                        <p v-else>-</p>
                    </div>
                </div>
                <div class="flex border-b border-40">
                    <div class="w-1/4 py-4">
                        <h4 class="font-normal text-80">{{__('ID Type')}}</h4>
                    </div>
                    <div class="w-3/4 py-4">
                        <p v-if="customer.id_type">{{ customer.id_type_string}}</p>
                        <p v-else>-</p>
                    </div>
                </div>
                <div class="flex border-b border-40">
                    <div class="w-1/4 py-4">
                        <h4 class="font-normal text-80">{{__('ID Number')}}</h4>
                    </div>
                    <div class="w-3/4 py-4">
                        <p v-if="customer.id_number">{{customer.id_number}}</p>
                        <p v-else>-</p>
                    </div>
                </div>

                <div class="flex border-b border-40" v-if="SHMS">
                    <div class="w-1/4 py-4">
                        <h4 class="font-normal text-80">{{__('ID Serial Number')}}</h4>
                    </div>
                    <div class="w-3/4 py-4">
                        <p v-if="customer.id_serial_number">{{customer.id_serial_number}}</p>
                        <p v-else>-</p>
                    </div>
                </div>

                <div class="flex border-b border-40" v-if="customer.customer_type == '3'">
                    <div class="w-1/4 py-4">
                        <h4 class="font-normal text-80">{{__('Visa Number')}}</h4>
                    </div>
                    <div class="w-3/4 py-4">
                        <p v-if="customer.visa_number">{{customer.visa_number}}</p>
                        <p v-else>-</p>
                    </div>
                </div>
                ...
                <div class="flex border-b border-40">
                    <div class="w-1/4 py-4">
                        <h4 class="font-normal text-80">{{__('Work')}}</h4>
                    </div>
                    <div class="w-3/4 py-4">
                        <p v-if="customer.work">{{customer.work }}</p>
                        <p v-else>-</p>
                    </div>
                </div>
                <div class="flex border-b border-40">
                    <div class="w-1/4 py-4">
                        <h4 class="font-normal text-80">{{__('Work Phone')}}</h4>
                    </div>
                    <div class="w-3/4 py-4">
                        <p v-if="customer.work_phone">{{customer.work_phone}}</p>
                        <p v-else>-</p>
                    </div>
                </div>
                <div class="flex border-b border-40">
                    <div class="w-1/4 py-4">
                    <h4 class="font-normal text-80">{{__('Address')}}</h4>
                    </div>
                    <div class="w-3/4 py-4">
                        <p v-if="customer.address" class="text-90">{{customer.address}}</p>
                        <p v-else class="text-90">-</p>
                    </div>
                </div>
                <div class="flex border-b border-40">
                    <div class="w-1/4 py-4">
                        <h4 class="font-normal text-80">{{__('ID Expire Date')}}</h4>
                    </div>
                    <div class="w-3/4 py-4">
                        <p v-if="customer.id_expire_date">{{customer.id_expire_date | formatDateSpecial}}</p>
                        <p v-else>-</p>
                    </div>
                </div>
                <div class="flex border-b border-40 remove-bottom-border">
                    <div class="w-1/4 py-4">
                        <h4 class="font-normal text-80">{{__('Birthday Date')}}</h4>
                    </div>
                    <div class="w-3/4 py-4">
                        <p v-if="customer.birthday_date">{{customer.birthday_date | formatDateSpecial}}</p>
                        <p v-else>-</p>
                    </div>
                </div>
            </div>
        ...
        <div class="content_page">
                            ...
                            <template v-if="reservations.length">
                                ...
                                <tr v-for="(reservation,i) in reservations" :key="i">
                                    ...
                                    <td class="td-fit text-right pr-6 flex items-center">
                                        <router-link :to="{path: `/reservation/${reservation.id}`}" :title="__('View')" class="cursor-pointer text-70 hover:text-primary mx-2">
            ...
            <div class="w-full flex flex-wrap mt-3 justify-center">
                <pagination
                    ...
                    :page-range="3"
                    :margin-pages="2"
                    :click-handler="getCustomerReservations"
    ...
    <notes v-permission="'view notes'" />

    <update-customer :highlights="highlights" :customer_obj="customer" :component_from="'show'" ref="editCustomer" />
...
export default {
  ...
  methods: {
    ...
    show(id) {
      this.isLoading = true;
      axios.get(`/nova-vendor/new/customers/${id}`)
      .then(response => {
    ...
    openEditCustomerModal(){
        setTimeout(() => {
            this.$refs.editCustomer.$refs.edit.open();
        }, 0);
    },
    getCustomerReservations(page=1){
        this.isLoadingReservations = true;
        axios.get(`/nova-vendor/new/customers/reservations/get?customer_id=${this.customer.id}&page=${page}`)
        .then(response => {
