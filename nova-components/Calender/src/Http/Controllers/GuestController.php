...
{
    public function filter(Request $request, Guest $customer)
    {
            ...
            switch ($request->get('search_attribute')) {
                ...
                case 'by_id_number':
                    if (auth()->user()->current_team_id == 44) {
                        $customers = collect($customer->where('team_id', 44)->searchByIDNumber1($search_string)->get()->take(30))->unique('id_number');
                    } else {
                        $customers = collect($customer->searchByIDNumber1($search_string)->get()->take(30))->unique('id_number');
                    }
                    break;
                case 'by_email':
                    if (auth()->user()->current_team_id == 44) {
                        $customers = collect($customer->where('team_id', 44)->searchByEmail1($search_string)->get()->take(30))->unique('email');
                    } else {
                        $customers = collect($customer->searchByEmail1($search_string)->get()->take(30))->unique('email');
                    }
                    break;
                case 'by_phone':
                    if (auth()->user()->current_team_id == 44) {
                        $customers = collect($customer->where('team_id', 44)->searchByPhone1($search_string)->get()->take(30))->unique('phone');
                    } else {
                        $customers = collect($customer->searchByPhone1($search_string)->get()->take(30))->unique('phone');
                    }

                    break;
                default:
                    if (auth()->user()->current_team_id == 44) {
                        $customers = collect($customer->where('team_id', 44)->searchByName1($search_string)->get()->take(30))->unique('name');
                    } else {
                        $customers = collect($customer->searchByName1($search_string)->get()->take(30))->unique('name');
                    }
                    break;
            }
            /**
    ...
    public function getCustomerInfo(Request $request, Guest $customer)
    {
