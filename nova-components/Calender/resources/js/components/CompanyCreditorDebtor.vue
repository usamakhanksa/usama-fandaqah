<template>
    ...
    <div v-if="reservation">
        <!-- <div class="customerNotes" @click="openCompanyReservationsModal">
            <span>
                <p v-if="total_balance > 0">{{__('The Company')}} <b class="text-green-500">{{__('Creditor')}}</b> - {{__('Show Reservations History')}}</p>
                <p v-if="total_balance < 0">{{__('The Company')}} <b class="text-red-500">{{__('Debtor')}}</b> - {{__('Show Reservations History')}}</p>
                <p v-if="total_balance == 0">{{__('Show Reservations History')}}</p>
            </span>
        </div> -->
        <sweet-modal :enable-mobile-fullscreen="false" :pulse-on-block="false" :title="modal_title" overlay-theme="dark" class="customer_notes_modal" ref="companyCreditorDebtorModal" @open="fetchData">
        
            <div class="content_page">
                <div class="notifier_table" >
                    <p v-if="total_balance > 0">{{ reservation && reservation.company && reservation.company.entity_type == 'company' ?  __('Total Company Balance') : __('Total Balance')}} : {{parseFloat(Math.abs(total_balance)).toFixed(2)}} <span v-if="(currency && currency == 'SAR') || !currency" class="icon-saudi_riyal"></span> <span v-else>{{ __(currency) }}</span>  <b class="text-green-500">{{__('Creditor')}}</b></p>
                    <p v-if="total_balance < 0">{{ reservation && reservation.company && reservation.company.entity_type == 'company' ?  __('Total Company Balance') : __('Total Balance')}} : {{parseFloat(Math.abs(total_balance)).toFixed(2)}} <span v-if="(currency && currency == 'SAR') || !currency" class="icon-saudi_riyal"></span> <span v-else>{{ __(currency) }}</span>  <b class="text-red-500">{{__('Debtor')}}</b></p>
                </div>
                    ...
                    <div class="table_responsive">
                        <table>
                            ...
                            <tr>
                                ...
                                <th>{{__('First Reservation Start Date')}}</th>
                                <th>{{__('Last Reservation End Date')}}</th>
                                <th>{{__('Balance')}}</th>
                                <th>{{__('Actions')}}</th>
                            </tr>
                            </thead>
                            <tbody>
                                <tr v-if="collection.length" v-for="(group_reservation,index) in collection" :key="index">
                                    <td>{{group_reservation.reservation_number}}</td>
                                    <td>{{group_reservation.units_count}}</td>
                                    <td>{{group_reservation.calculations.start_date}}</td>
                                    <td>{{group_reservation.calculations.end_date}}</td>
                                    <td>
                                        <span v-if="group_reservation.balance > 0" class="creditor_cls text-green-500">{{parseFloat(Math.abs(group_reservation.balance)).toFixed(2)}} <span v-if="(currency && currency == 'SAR') || !currency" class="icon-saudi_riyal"></span> <span v-else>{{ __(currency) }}</span>  ({{__('Creditor')}})</span>
                                        <span v-else-if="group_reservation.balance < 0" class="debitor_cls text-red-500">{{parseFloat(Math.abs(group_reservation.balance)).toFixed(2)}} <span v-if="(currency && currency == 'SAR') || !currency" class="icon-saudi_riyal"></span> <span v-else>{{ __(currency) }}</span>  ({{__('Debtor')}})</span>
                                        <span class="normal_cls" v-else>0.00 <span v-if="(currency && currency == 'SAR') || !currency" class="icon-saudi_riyal"></span> <span v-else>{{ __(currency) }}</span> </span>
                                    </td>
                                    <td>
                                        <div class="action">
                                            <button v-if="group_reservation.id != reservation.id" :title="__('View')" @click="closeModal(group_reservation.id)">
                                            ...
                                            <span class="normal_cls text-gray-500" v-if="group_reservation.id == reservation.id">{{__('Currently showing')}}</span>
                                        </div>
                                    </td>
                                </tr>
                                <tr v-else>
                                    <td colspan="4">{{__('No Reservations Found')}}</td>
                                </tr>
                            </tbody>
                        </table>
                        <br>
                        <div >
                            <pagination
                                v-if="collection.length && paginator.totalResults > 10"
                                :page-count="paginator.lastPage"
                                :page-range="3"
                                :margin-pages="2"
                                :click-handler="getCompanyReservations"
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            closeModal(reservation_id){
                this.$refs.companyCreditorDebtorModal.close();
                Nova.$emit('get-reservation' , reservation_id);
                this.$router.push(`/reservation/${reservation_id}`);
            },
            getCompanyReservations(page = 1){
                this.isLoading = true;
                Nova.request().get(`/nova-vendor/calender/company-creditor-debtor?company_id=${this.reservation.company_id}&page=${page}`)
                    .then(response => {


                        this.collection = response.data.data;
                        this.total_balance = response.data.grand_group_balance;
                        this.paginator = {
            ...
            // openCompanyReservationsModal(){
            //     this.getCompanyReservations();
            //     this.$refs.companyCreditorDebtorModal.open();
            // },
        },
        mounted() {

            this.route_name = this.$route.name;
            Nova.$on('clear-collection' , () => {
                this.collection = [];
                this.reservations_count = 0;
            })
            if(this.reservation.company.entity_type == 'individual'){
                this.modal_title = Nova.app.__('Customer Reservations History :customer' , {customer : this.reservation.company.name });
            }else{
                this.modal_title = Nova.app.__('Company Reservations History :company_name' , {company_name : this.reservation.company.name });
            }
        }
    }
</script>
