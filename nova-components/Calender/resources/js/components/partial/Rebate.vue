<template>
    <div>
      ...
      <button v-if="!quick && !reservation.checked_out && (reservation.status == 'confirmed' || reservation.status == 'awaiting-payment' || reservation.status == 'awaiting-confirmation')" v-permission="'add rebates'" @click="open" class="cash_receipt_button">{{__('Rebate')}}</button>
      <button v-if="!quick && occ" v-permission="'add rebates'" @click="open" class="cash_receipt_button">{{__('Rebate')}}</button>
      <sweet-modal :enable-mobile-fullscreen="false" class="cash_receipt_modal" :pulse-on-block="false" :title="__('Rebate')" overlay-theme="dark" ref="modal">
          ...
          data: () => {
              return {
                  ...
                  config: {
                    ...
                    disableMobile: true,
                    minuteIncrement : 1,
                    clickOpens : Nova.app.$hasPermission('change transactions date') && Nova.app.$hasPermission('add rebates') ? true : false
                  },    
                  from: null,
                  termsLoading: true,
                  terms: {},
