<template>
  <div class="special_invoice">
    ...
    <sweet-modal :enable-mobile-fullscreen="false" :pulse-on-block="true" :title="__('Update note on reservation')" overlay-theme="dark" ref="updateNote" @open="fillNote" class="add_invoice_modal">
      <loading :active.sync="isLoading" :is-full-page="false"></loading>
      <!-- <div class="share_button_reservation">
          <button class="trash_button" @click="openDeleteNote(comment.id)"></button>
        </div>  -->
      <div class="inputs_row">
        <div class="big_col">
          <textarea name="notes" id="" cols="30" rows="3" v-model="noteOnReservation"></textarea>
        </div>
      </div><!-- inputs_row -->
      <button class="save_button" @click="updateNote">{{__('Save')}}</button>
    </sweet-modal>

    <!-- <delete-confirm-component :id="comment.id" ref="deleteNoteHaandler" /> -->
  </div><!-- special_invoice -->
...
<script>
    ...
    export default {
        ...
        data(){
            return {
                isLoading : false,
                noteOnReservation : null
            }
        },
        methods:{
            fillNote(){
                this.noteOnReservation = this.comment.comment;
            },

            updateNote(){
                this.isLoading = true;
                if(!this.noteOnReservation){
                    this.$toasted.show(this.__('Note content can not be empty'), {type: 'error'});
                    this.isLoading = false;
                    return ;
                }
                axios
                    .put('/nova-vendor/calender/comments', {
                        comment_id : this.comment.id,
                        commentable_type: "App\\Reservation",
                        commentable_id: this.comment.commentable_id,
                        comment: this.noteOnReservation,
                    })
                    .then(res => {
                        this.isLoading = false;
                        if(res.data.success) {
                            this.$refs.updateNote.close();
                            Nova.$emit('note-on-reservation-updated');
                            this.$toasted.show(this.__('Note on reservation updated successfully'), {type: 'success'});
                        }
                    })
            },
            openDeleteNote(){
                ...
                setTimeout(() => {
                    this.$refs.deleteNoteHaandler.$refs.deleteNote.open();
                }, 0);
            }
