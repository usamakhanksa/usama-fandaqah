<template>
  <div class="companies_component">
        <sweet-modal :enable-mobile-fullscreen="false" :pulse-on-block="false" :title="__('Add Company')" overlay-theme="dark" ref="convertReservationModal" class="convert_to_group">
          ...
          <div class="search_company">
            ...
            <div class="label_with_button mb-2">
                ...
                <button  v-permission="'create companies'" @click="openAddCompanyModal">{{__('Add Company Now')}}</button>
            </div>
            <div class="search_company">
                <autocomplete
            ...
            <button style="min-width: 100%;"  @click="convertReservation">{{__('Update')}}</button>
            </div>
        </sweet-modal>
...
<script>
    ...
    export default {
        name: "convert-to-group-reservation",
        components : {
            Loading,
            CompanyValidation
        },
        ...
        methods: {
            convertReservation(){
                if(!this.company_id){
                    this.$toasted.show(this.__("Please select / add a company"), {type: 'error'})
                    return
                }
                this.loading = true;
                axios.post('/nova-vendor/calender/convert-to-group-reservation' , {
                    company_id : this.company_id,
                    reservation_id : this.reservation.id,
                    reservation_type : this.reservation.reservation_type
                })
                .then(response => {
                    if(response.data.success && response.data.reload_reservation){
                        this.clearInputs();
                        this.loading = false;
                        this.disabled = false;
                        this.showErrorBag = false;
                        this.$refs.convertReservationModal.close();
                        this.$toasted.show(this.__('Company added successfully'), {type: 'success'});
                        Nova.$emit('reservation_converted_to_group');
                    }
                })
            },
