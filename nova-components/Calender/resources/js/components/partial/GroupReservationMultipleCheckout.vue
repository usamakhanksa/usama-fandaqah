<template>
    <!-- <div> -->
      <div class="item_reservation_button"  v-if="reservation.checked_in">
          <button v-permission="'check-out customer'" v-if="reservation.checked_in" class="main_button action-btn" @click="openGroupReservationCheckoutModal">{{__('Check out Group Reservation')}}</button>
      ...
      <sweet-modal 
          ...
          width="70%" 
          :title="__('Check out Group Reservation')" 
          overlay-theme="dark" 
          ref="groupReservationCheckoutRef" 
          class="summary_modal"
      >
          <div id="update_prices_div" class="relative">
              ...
              />
              <div class="title">
                  {{__('Available Reservations To Make Checkout')}}
              </div>
              ...
              <div class="content_page relative">
                  <div class="overflow-hidden overflow-x-auto relative">
                      <div class="group_reservations_tbl rounded overflow-hidden">
                        ...
                        <div class="alert_balance" v-if="total_balance != 0">
                            <div>
                            {{ __('You have not filtered for this booking - Balance') }} :
                            <span :class="{ 'text-success': total_balance > 0 ,'text-danger': total_balance < 0 }">
                                {{ Math.abs(total_balance).toFixed(2) + ' ' + __(currency)}}
                                <p :class="{ 'text-success': total_balance > 0 ,'text-danger': total_balance < 0 }">{{ (total_balance  > 0) ? '('+__('credit')+')'  : ''}} {{ (total_balance  < 0) ? '('+__('debit')+')' : ''}}</p></span>
                            </div>
                        </div>
    
                        <div class="input-group p-1">
                            <label v-if="automatic_under_cleaning" class="cleaning_msg">
                                <div>
                                    <span>{{__('Units status will be changed to under cleaning automatically')}}</span>
                                </div>
                            </label><!-- cleaning_msg -->
                        </div>
                        <div class="input-group p-1">
                            <label class="text-90">{{__('Check-out time')}} </label>
                          ...
                          >
                              ...
                              <tr>
                                  <th>{{__('Statement')}}</th>
                                  <th v-if="!automatic_under_cleaning" @click="checkAllUnits" style="cursor: pointer;">{{ __('Cleaning - Check All Units') }} <input type="checkbox" v-model="checkUnits"> </th>
                                  <th @click="checkAllReservations" style="cursor: pointer;">{{ __('Check All') }} <input v-if="reservations.length" type="checkbox" v-model="checkAll"> </th>
                              </tr>
                              </thead>
                              <tbody>
                                    ...
                                    <tr v-for="(reservation,key) in reservations" :key="key">
                                        ...
                                        <td v-if="!automatic_under_cleaning">
                                            <label for="">{{ reservation.unit.name[locale] }} - {{ reservation.unit.unit_number }}</label>
                                            <input type="checkbox" :value="reservation.unit_id" v-model="checked_units">
                                        </td>
                                        <td>
                                    ...
                                    <template v-else>
                                        <tr>
                                            <td :colspan="automatic_under_cleaning ? 2 : 3">{{ __('No Reservations Found') }}</td>
                                        </tr>
                                    </template>
                              </tbody>
                          </table>
                      </div>
                      <button  class="shadow mb-4  btn btn-block btn-primary mt-2" @click="doCheckout">{{__('Check Out')}}</button>
                  </div>
              </div>
          </div>
      </sweet-modal>
  ...
  <script>
      ...
      export default {
          name : 'group-reservation-multiple-checkout',
          props: ["reservation"],
          components: {
              Loading,
          ...
          data() {
              return {
                  ...
                  last_reservation_to_checkout : null,
                  currency :Nova.app.currentTeam.currency
              }
          },
          methods: {
         
           async openGroupReservationCheckoutModal() {
                    this.checkAll = false;
                    this.isLoading = true;
                    ...
                    // }  
                  this.getQualifiedForCheckoutReservations();  
                  this.getAutomaticUnderCleaningStatus();
                  this.$refs.groupReservationCheckoutRef.open()
            },
            getQualifiedForCheckoutReservations(){
        
                this.isLoading = true;
                axios.get(`/nova-vendor/calender/get-qualified-for-checkin-reservations?main_id=${this.reservation.attachable_id ? this.reservation.attachable_id  : this.reservation.id}&time=${this.time}&type=checkout`)
                .then( response => {
            ...
            doCheckout(){
                ...
                if(!this.checked_reservations.length){
                    this.isLoading = false;
                    this.$toasted.show(this.__('Can not check-out , please select at least one reservation'), {type: 'error'});
                    return;
                }

                axios.post(`/nova-vendor/calender/doCheckoutGroupReservations`, {
                    checked_reservations : this.checked_reservations,
                    checked_units : this.checked_units,
                    automatic_under_cleaning : this.automatic_under_cleaning,
                    time : this.time,
                    main_id : this.reservation.attachable_id ? this.reservation.attachable_id  : this.reservation.id
                })
                .then(response => {
                    if(response.data.success && response.data.simple_checkout){
                        this.$toasted.show(this.__('Checkout for selected reservations went successfully and there is still pending reservations'), {type: 'success'});
                        this.$refs.groupReservationCheckoutRef.close();
                        Nova.$emit('close-toggle-group-buttons');
                        this.checked_reservations = [];
                        this.checked_units = [];
                        this.checkAll = false;
                        this.checkUnits = false;
                        this.automatic_under_cleaning = false;
                        this.isLoading = false;
                        Nova.$emit('refresh-reservation');
                    }

                    if(response.data.success && !response.data.simple_checkout && response.data.is_last){
                    
                       if(response.data.deposit_insurance_transactions & !response.data.withdraw_insurance_transactions && response.data.show_insurance_retrieval_modal){
                            // Nova.$emit('add-retrieval-insurance-for-group-reservation' , response.data.deposit_insurance_transaction);
                            this.last_reservation_to_checkout = response.data.last_reservation
                            Nova.$emit('add-retrieval-insurance-for-group-reservation-from-group-checkout', {
                                deposit_transaction : response.data.deposit_insurance_transaction,
                                reservation : this.last_reservation_to_checkout,
                                shared_invoices : response.data.shared_invoices,
                                automatic_under_cleaning : this.automatic_under_cleaning,
            ...
            checkAllUnits(){
                this.checkUnits = !this.checkUnits;
                if(!this.checkUnits){
                    this.checked_units = [];
                }
                const self = this;
                this.reservations.forEach(reservation => {
                    if(self.checkUnits){
                        this.checked_units.push(reservation.unit_id);
                    }
                });
                
            },
            getAutomaticUnderCleaningStatus(reservation_id,unit_id){
                axios.get(`/nova-vendor/calender/check-convert-unit-to-under-cleaning?unit_id=${unit_id}&reservation_id=${reservation_id}`)
                    .then(response => {
