<template>
  <div>
    <sweet-modal :enable-mobile-fullscreen="false" :pulse-on-block="true" :title="__('Invoices')" overlay-theme="dark" ref="invoicesListModal" class="invoices_list_modal">
            ...
            <template v-if="invoices.length">
              <tr v-for="(invoice,i) in invoices" :key="i">
                ...
                <td class="actions">
                  ...
                  <a v-if="!invoice.invoice_credit_note && currentInOrderCreditNoteId === invoice.id && (reservation.checked_out === null || openClosedContract)" class="add_credit_note_button" @click="!can_add_credit_note ? '#' : openConfirmAddCreditNote(invoice)"></a>
    ...
    <invoice ref="invoiceListComponent" :invoice="invoice" :occ="openClosedContract"  :isApplicableToAddCreditNote="isApplicableToAddCreditNote" :reservation="reservation" />
    <!-- <invoice-delete-confirm :invoice="invoice" ref="deleteButton" :ref-parent="'invoicesListModal'"/> -->
    <invoice-credit-note-confirm :invoice="targetInvoice" ref="addCreditNoteButton" />
    <invoice-credit-note v-if="injectCreditNoteModal"  :reservation="reservation" :invoice="targetInvoice" :credit_note="targetInvoice.invoice_credit_note" :locale="local" ref="creditNote" />
...
<script>
    ...
    export default {
        name: "InvoicesList",
        components: {InvoiceDeleteConfirm, Invoice,InvoiceCreditNoteConfirm,InvoiceCreditNote},
        data(){
            return {
                invoices : [],
                invoice : {},
        ...
        methods : {
            openInvoiceModal(invoice){
                ...
                this.$refs.invoiceListComponent.$refs.invoiceModal.open();
            },
        ...
        mounted() {
            ...
            Nova.$on('credit-note-added' , ()=> {
                this.$refs.invoicesListModal.close();
            })
