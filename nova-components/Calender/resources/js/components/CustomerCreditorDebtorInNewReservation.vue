<template>
    ...
    <div>
        ...
        <sweet-modal v-if="customer" :enable-mobile-fullscreen="false" :pulse-on-block="false" :title="__('Customer Reservations History :customer' , {customer : customer.name })" overlay-theme="dark" class="customer_notes_modal" ref="customerCreditorDebtorModal" @open="fetchData">
            <div class="content_page">
                <div class="notifier_table">
                    <p v-if="total_balance > 0">{{__('Total Customer Balance')}} : {{parseFloat(Math.abs(total_balance)).toFixed(2)}} {{__(currency)}} <b class="text-green-500">{{__('Creditor')}}</b></p>
                    <p v-if="total_balance < 0">{{__('Total Customer Balance')}} : {{parseFloat(Math.abs(total_balance)).toFixed(2)}} {{__(currency)}} <b class="text-red-500">{{__('Debtor')}}</b></p>
                    <p v-if="total_balance == 0">{{__('Total Customer Balance')}} : 0 {{__(currency)}}</p>
                </div>
                        ...
                        <table>
                            ...
                            <tbody>
                                <tr v-if="collection.length" v-for="(res_obj,index) in collection" :key="index">
                                    ...
                                    <td>
                                        <span v-if="res_obj.balance > 0" class="creditor_cls text-green-500">{{parseFloat(Math.abs(res_obj.balance)).toFixed(2)}} {{__(currency)}} ({{__('Creditor')}})</span>
                                        <span v-else-if="res_obj.balance < 0" class="debitor_cls text-red-500">{{parseFloat(Math.abs(res_obj.balance)).toFixed(2)}} {{__(currency)}} ({{__('Debtor')}})</span>
                                        <span class="normal_cls" v-else>{{parseFloat(Math.abs(res_obj.balance)).toFixed(2)}} {{__(currency)}}</span>
                                    </td>
                                    <td>
                                        <div class="action">
                                            <button :title="__('View')" @click="closeModal(res_obj.id)">
...
<script>
    ...
    export default {
        name: "CustomerCreditorDebtorInNewReservation",
        components : {
            Loading
        },
        props : ['customer'],
        ...
        methods:{
            ...
            getCustomerReservationsHistory(page = 1){
                this.isLoading = true;
                Nova.request().get(`/nova-vendor/calender/customer-creditor-debtor?id=${this.customer.id}&page=${page}`)
                    .then(response => {
