<template>
    ...
    <div>
        <!-- <div class="customerNotes" v-if="reservations_count > 1">
        ...
        <sweet-modal :enable-mobile-fullscreen="false" :pulse-on-block="false" :title="__('Customer Reservations History :customer' , {customer : customer_name })" overlay-theme="dark" class="customer_notes_modal" ref="customerCreditorDebtorModal" @open="fetchData">
            <div class="content_page">
                <div class="notifier_table">
                    <p v-if="total_balance > 0">{{__('Total Customer Balance')}} : {{parseFloat(Math.abs(total_balance)).toFixed(2)}} <span v-if="(currency && currency == 'SAR') || !currency" class="icon-saudi_riyal"></span> <span v-else>{{ __(currency) }}</span> <b class="text-green-500">{{__('Creditor')}}</b></p>
                    <p v-if="total_balance < 0">{{__('Total Customer Balance')}} : {{parseFloat(Math.abs(total_balance)).toFixed(2)}} <span v-if="(currency && currency == 'SAR') || !currency" class="icon-saudi_riyal"></span> <span v-else>{{ __(currency) }}</span> <b class="text-red-500">{{__('Debtor')}}</b></p>
                    <p v-if="total_balance == 0">{{__('Total Customer Balance')}} : 0 <span v-if="(currency && currency == 'SAR') || !currency" class="icon-saudi_riyal"></span> <span v-else>{{ __(currency) }}</span></p>
                </div>
                <div class="table_area">
                    ...
                    <div class="table_responsive">
                        <table>
                            ...
                            <tbody>
                                <tr v-if="collection.length" v-for="(res_obj,index) in collection" :key="index">
                                    <td>{{res_obj.reservation_number}}</td>
                                    <td>{{res_obj.unit}}</td>
                                    <td>{{res_obj.date_in}}</td>
                                    <td>{{res_obj.date_out}}</td>
                                    <td>{{ res_obj.reservation_type == 'single' ? __('Single') : __('Group') }}</td>
                                    <td>
                                        <span v-if="res_obj.balance > 0" class="creditor_cls text-green-500">{{parseFloat(Math.abs(res_obj.balance)).toFixed(2)}} <span v-if="(currency && currency == 'SAR') || !currency" class="icon-saudi_riyal"></span> <span v-else>{{ __(currency) }}</span> ({{__('Creditor')}})</span>
                                        <span v-else-if="res_obj.balance < 0" class="debitor_cls text-red-500">{{parseFloat(Math.abs(res_obj.balance)).toFixed(2)}} <span v-if="(currency && currency == 'SAR') || !currency" class="icon-saudi_riyal"></span> <span v-else>{{ __(currency) }}</span> ({{__('Debtor')}})</span>
                                        <span class="normal_cls" v-else>{{parseFloat(Math.abs(res_obj.balance)).toFixed(2)}} <span v-if="(currency && currency == 'SAR') || !currency" class="icon-saudi_riyal"></span> <span v-else>{{ __(currency) }}</span></span>
                                    </td>
                                    <td>
                                        <div class="action">
                                            <button v-if="res_obj.id != reservation.id" :title="__('View')" @click="closeModal(res_obj.id)">
                                            ...
                                            <span class="normal_cls text-gray-500" v-else>{{__('Currently showing')}}</span>
                                        </div>
                                    </td>
                                </tr>
                                <tr v-else>
                        ...
                        <div >
                            <pagination
                                ...
                                :page-range="3"
                                :margin-pages="2"
                                :click-handler="getCustomerReservationsHistory"
    ...
    export default {
        ...
        methods:{
            ...
            fetchData(){
                ...
                this.getCustomerReservationsHistory();
            },

            closeModal(id){
                this.$refs.customerCreditorDebtorModal.close();
                if(this.reservation){
                    Nova.$emit('get-reservation' , id);
                }
                this.$router.push(`/reservation/${id}`);
            },
            getCustomerReservationsHistory(page = 1){
                this.isLoading = true;
                Nova.request().get(`/nova-vendor/calender/customer-creditor-debtor?id=${this.reservation.customer_id}&page=${page}`)
                    .then(response => {

                        this.collection = response.data.data;
                        this.customer_name = response.data.customer_name;
                        this.customer_team_id = response.data.customer_team_id;
                        this.paginator = {
