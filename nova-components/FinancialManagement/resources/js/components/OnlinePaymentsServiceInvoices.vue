<template>
    <div>
        ...
        <div id="onlinePaymentsManagement">
            <div class="title">{{ __('Payments Service Invoices') }}</div>
                ...
                <div class="filter_area">
                    ...
                    <ul>
                        <li>
                            ...
                            <p>{{ calculations.total_fees }} {{__(currency)}}</p>
                        </li>

                        <li>
                            <span>{{__('Vat On Fees')}}</span>
                            <p>{{ calculations.vat_on_fees }} {{__(currency)}}</p>
                        </li>

                        <li>
                            <span>{{__('Total')}}</span>
                            <p>{{ calculations.total_amount }} {{__(currency)}}</p>
                        </li>
                    ...
                    <div class="table-responsive relative">
                        ...
                        >
                            ...
                            <tr>
                                ...
                                <!-- <th>{{__('File Name')}}</th> -->
                                <th>{{__('Total Fees')}} ({{ __(currency) }}) </th>
                                <th>{{__('Vat On Fees')}} ({{ __(currency) }}) </th>
                            ...
                            <template v-if="data.length">
                                <tr v-for="(obj,index) in data" :key="index">
                                    ...
                                    <!-- <td>{{ obj.file_name }}</td> -->
                                    <td>{{ obj.total_fees.toFixed(2) }}</td>
                                    <td>{{ obj.vat_on_fees.toFixed(2) }}</td>
                                    
                                    <td>{{obj.created_at | formatDateWithAmPm}}</td>
                                    
                                    <td class="td-fit">
                                        <a target="_blank" :href="obj.driver == 's3' ? 'https://fandaqah.s3.amazonaws.com/' + obj.file_path + obj.file_name : local_url + '/' + obj.file_path + obj.file_name ">
                                            <img style="width: 30px; cursor: pointer;" src="../../../../../public/fonts/file-download.svg" alt="">
                                        </a>
                                    </td>
                                </tr>
                            </template>
                            <template v-else>
                                <tr>
                                    <td colspan="15" class="text-center">{{__('No Online Payments Service Invoices Found')}}</td>
                                </tr>
                            </template>
...
export default {
    ...
    data(){
        return {
            ...
            calculations : {
                total_fees : (0).toFixed(2),
                vat_on_fees : (0).toFixed(2),
    ...
    methods:{
        ...
        getAllData(){
            ...
            axios.get(`/nova-vendor/financial-management/payment/get-service-invoicing` , config)
            .then(response => {
                var invoices = response.data.data;
                if(invoices.length){

                    let total_fees = _.sumBy(invoices.filter((invoice) => invoice.total_fees), 'total_fees');
                    this.calculations.total_fees = total_fees.toFixed(2);

                    let vat_on_fees = _.sumBy(invoices.filter((invoice) => invoice.vat_on_fees), 'vat_on_fees');
                    this.calculations.vat_on_fees = vat_on_fees.toFixed(2);

                    this.calculations.total_amount = (total_fees+vat_on_fees).toFixed(2);

                }else{
                    this.calculations.total_fees = (0).toFixed(2);
                    this.calculations.vat_on_fees = (0).toFixed(2);
            ...
            axios.get(`/nova-vendor/financial-management/payment/get-service-invoicing` , config)
                .then((response) => {
                    ...
                    this.paginator = {
                        ...
                        firstPageUrl : response.data.first_page_url || null ,
                        lastPageUrl : response.data.last_page_url || null ,
        ...
        getServiceInvoices(){
            alert('fetching service invoices');
        }
    },
    watch:{
    ...
    mounted() {
        ...
        this.crumbs = [
            ...
            {
                text : 'Payments Service Invoices',
                to : '#'
            }
        ];
        this.query = this.$route.query;
