<template>
    <div>
        ...
        <div id="creditNotesManagement">
            <div class="title">{{ __('Credit Notes Management') }}</div>
            <div class="content_page">
                ...
                <div class="action_buttons">
                    <div class="buttons_area" v-if="data.length">
                        ...
                        <button type="button" class="print_button" @click="printCreditNotes"></button>
                    </div><!-- buttons_area -->
                </div><!-- action_buttons -->
                    ...
                    <div class="table-responsive relative">
                        ...
                        >
                            ...
                            <tr>
                                ...
                                <th>{{__('Credit Note Creation Date')}}</th>
                                <th>{{__('Invoice Creation Date')}}</th>
                                <th>{{__('Period From')}}</th>
                                <th>{{__('Period To')}}</th>
                                <th v-permission = "'print invoices'">{{__('Actions')}}</th>
                            </tr>
                            </thead>
                            <tbody>
                            <template v-if="data.length">
                                <tr v-for="(obj,index) in data" :key="index">
                                    ...
                                    <template v-if="!obj.invoice">
                                       <td v-if="obj.payload">{{ getParsedPayload(obj.payload).inv_number }}</td>
                                    </template>
                                    <template v-else>
                                        <td>{{obj.invoice ? obj.invoice.number : '-'}}</td>
                                    </template>
                                     <td v-if="obj.invoice && obj.invoice.reservation">
                                        <router-link v-if="!obj.invoice.reservation.deleted_at" :to="{name : 'reservation' , params : {id :  obj.invoice.reservation.id }}" class="text-info-dark">{{obj.invoice.reservation.number}}</router-link>
                                        <p v-else>{{obj.invoice.reservation.number}}</p>
                                    </td>
                                    <td v-else>-</td>

                                    <template v-if="!obj.invoice">
                                        <td v-if="obj.payload && getParsedPayload(obj.payload).customer_name">{{ getParsedPayload(obj.payload).customer_name }}</td>
                                        <td v-else>-</td>
                                    </template>
            
                                    <template v-else>
                                        <td v-if="obj.invoice && obj.invoice.reservation && obj.invoice.reservation.customer && !obj.invoice.reservation.company">{{obj.invoice.reservation && obj.invoice.reservation.customer  ? obj.invoice.reservation.customer.name : '-'}}</td>
                                        <td v-else>{{obj.invoice && obj.invoice.reservation.company.name ? obj.invoice.reservation.company.name : '-'}}</td>
                                    </template>
                                  
                                    <td>{{obj.employee ? obj.employee.name : '-'}}</td>
                                    <template v-if="!obj.invoice">
                                        <td v-if="obj.payload && getParsedPayload(obj.payload).total_with_taxes">{{ getParsedPayload(obj.payload).total_with_taxes }}</td>
                                    </template>
                                    <template v-else>
                                        <td>{{obj.invoice ? obj.invoice.amount : '-'}}</td>
                                    </template>
                                    <td>{{obj.created_at}}</td>
                                    <template v-if="!obj.invoice">
                                        <td>{{getParsedPayload(obj.payload).inv_created_at}}</td>
                                    </template>
                                    <template v-else>
                                        <td>{{obj.invoice ? obj.invoice.created_at : '-'}}</td>
                                    </template>
                                    <td>{{obj.invoice ? obj.invoice.period_from : '-'}}</td>
                                    <td>{{obj.invoice ? obj.invoice.period_to : '-'}}</td>
                                    <td v-permission = "'print invoices'" class="td-fit">


                                        <a
                                            v-if="obj && !obj.invoice"
                                            :href="'/home/reservation/credit-note/' +  obj.hash_id + '?nov=true'"
                                            target="_blank"
                                            v-tooltip="{
                                                targetClasses: ['it-has-a-tooltip'],
                                                placement: 'top',
                                                content: __('Print Credit Note'),
                                                classes: ['tooltip_reset']
                                            }"
                                        >
                                           <svg enable-background="new 0 0 32 32" id="Stock_cut" version="1.1" viewBox="0 0 32 32" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"><desc/><g><path d="M26,23h1   c1.105,0,2-0.895,2-2V9c0-1.105-0.895-2-2-2H5C3.895,7,3,7.895,3,9v12c0,1.105,0.895,2,2,2h1" fill="none" stroke="#000000" stroke-linejoin="round" stroke-miterlimit="10" stroke-width="2"/><line fill="none" stroke="#000000" stroke-linejoin="round" stroke-miterlimit="10" stroke-width="2" x1="6" x2="26" y1="19" y2="19"/><rect fill="none" height="12" stroke="#000000" stroke-linejoin="round" stroke-miterlimit="10" stroke-width="2" width="14" x="9" y="19"/><rect fill="none" height="6" stroke="#000000" stroke-linejoin="round" stroke-miterlimit="10" stroke-width="2" width="14" x="9" y="1"/><line fill="none" stroke="#000000" stroke-linejoin="round" stroke-miterlimit="10" stroke-width="2" x1="26" x2="24" y1="11" y2="11"/><line fill="none" stroke="#000000" stroke-linejoin="round" stroke-miterlimit="10" stroke-width="2" x1="12" x2="20" y1="23" y2="23"/><line fill="none" stroke="#000000" stroke-linejoin="round" stroke-miterlimit="10" stroke-width="2" x1="12" x2="20" y1="27" y2="27"/></g></svg>
                                        </a>
                                        <a
                                            v-if="obj && obj.invoice && obj.invoice.reservation.reservation_type == 'single'"
                                            :href="'/home/reservation/credit-note/' +  obj.hash_id"
                                            target="_blank"
                                            v-tooltip="{
                                                targetClasses: ['it-has-a-tooltip'],
                                                placement: 'top',
                                        ...
                                        <a
                                            v-if="obj && obj.invoice && obj.invoice.reservation.reservation_type == 'group'"
                                            :href="`/home/group_reservation/company_invoice_credit_note/${obj.hash_id}?locale=${locale}`"
                                            target="_blank"
                                            v-tooltip="{
                                                targetClasses: ['it-has-a-tooltip'],
                                                placement: 'top',
                                        ...
                                        >
                                          ...
                                          <a
                                            v-if="obj.invoice && obj.invoice.reservation.reservation_type == 'single'"
                                            :href="obj.invoice.print_url"
                                            target="_blank"
                                            v-tooltip="{
                                                targetClasses: ['it-has-a-tooltip'],
                                                placement: 'top',
                                                content: __('Print Invoice'),
                                                classes: ['tooltip_reset']
                                            }"
                                        >
                                        ...
                                        <a
                                            v-else
                                            v-show="obj.invoice && obj.invoice.reservation.reservation_type == 'group'"
                                            :href="`/home/group-reservation/company-live-invoice?inv=${obj.invoice ? obj.invoice.hash_id : ''}&locale=${locale}`"
                                            target="_blank"
                                            v-tooltip="{
                                                targetClasses: ['it-has-a-tooltip'],
                                                placement: 'top',
                            ...
                            <template v-else>
                                <tr>
                                    <td colspan="11" class="text-center">{{__('No Credit Notes Found')}}</td>
                                </tr>
                            </template>
...
export default {
    ...
    methods:{
        getParsedPayload(jsonString) {
            try {
                // This is the key: JSON.parse() converts the string to an object
        ...
        excelExport(){
            ...
            axios.post(window.FANDAQAH_API_URL + '/reservations/credit-notes-report-excel' , {
        ...
        getData(){
            ...
            axios.get(window.FANDAQAH_API_URL + `/reservations/credit-notes-report` , config)
                .then((response) => {
    ...
    watch:{
        ...
        invoiceNumber: function (val) {
            if(val != null){
                let opt = {}
                opt["filter[by_invoice_number]"] = val;
                opt["page"] = 1;
                this.$router.push({
                    name : 'credit-notes',
                    query: Object.assign({}, this.$route.query, opt)
