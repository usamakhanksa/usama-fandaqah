<template>
  <div id="unit_general_features_page">
    <div class="title">
      <span>{{__('General Features')}}<p>( {{paginator ? paginator.totalResults : 0}} )</p></span>
<!--      <a onclick="Nova.app.$router.push('/resources/unit-general-features/new?viaResource=&viaResourceId=&viaRelationship=')">{{__('Add a public feature')}}</a>-->
    </div><!-- title -->
        ...
        <table>
          ...
          <tbody>
            <template v-if="collection.length">
              <tr v-for="(feature , i) in collection" :key="i">
                <td>{{feature.name}}</td>
                <td>
                  <span v-if="feature.status" class="enabled">{{__('Active')}}</span>
                  <span v-else class="not_enabled">{{__('Inactive')}}</span>
                </td>
                <td>{{feature.order}}</td>
                <td>
                  <div class="action">
                    <router-link :to="`/resources/unit-general-features/${feature.id}`" :title="__('View')">
                    ...
                    <router-link :to="`/resources/unit-general-features/${feature.id}/edit?viaResource=&viaResourceId=&viaRelationship=`" :title="__('Edit')">
                    ...
                    <button type="button" :title="__('Delete')" @click="initDeleteConfirm(feature.id)" v-if="feature.unit_categories_count == 0">
            ...
            <template v-else>
              <tr>
                <td colspan="7">{{__('No General Features Matches Your Criteria')}}</td>
              </tr>
            </template>
          </tbody>
        </table>
  ...
  </div><!-- unit_general_features_page -->
</template>
...
<script>
    ...
    export default {
        ...
        mounted(){
            ...
            Nova.$on('UnitGeneralFeature-destroyed' , () => {
                this.getData(1);
            });
        },
        created(){
