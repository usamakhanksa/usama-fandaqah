<template>
    ...
    <div class="relative">
        ...
        <div class="title">
            <span>{{__('Reservations awaiting payment')}} ({{oRPaginator ? oRPaginator.total : 0}})</span>
            <div class="icon"></div>
        </div><!-- title -->
        <div class="content_area">
            ...
            <div class="no_data_show" v-if="!oRCollection.length">
                <div class="icon"></div>
                <span>{{__('There are no reservations awaiting payment')}}</span>
            </div><!-- no_data_show -->
            <!-- Show only if there are no reservations -->
...
<script>
    ...
    export default {
        name: "panel-awaiting-payment-reservations",
        components : {
            Loading
        },
        data(){
        ...
        created() {
            ...
            Nova.$on('call-awaiting-payment-reservations-query' , () => {
                this.getReservations();
            })
        },
        beforeDestroy(){
            Nova.$off('call-awaiting-payment-reservations-query')
        }
    }
</script>
